...
**Behavior change:** an unguarded `{children()}` on a component mounted without children now throws at render, where it previously rendered nothing. Declare `children: Snippet` as a required (non-optional) prop and `abide check` catches the childless mount at the call site — slotted content (`<Card>text</Card>`) satisfies the requirement, while a bare `<Card />` is flagged. If a component is meant to work both with and without children, declare `children?: Snippet` (optional) and guard the render with `{#if children}{children()}{:else}…{/if}`.

Migration: add `import { props } from '@abide/abide/ui/props'` to every component that calls `props()`; for any component that reads `children`, add `import type { Snippet } from '@abide/abide/shared/snippet'` and declare it — `const { children } = props<{ children: Snippet }>()` when children are always expected (call sites are checked), or `children?: Snippet` when the component may be mounted childless.
