// The browser gate: two projects over the one app, each with its own server.
//
...
//
// Two projects for ONE app, which is the shape now that the use cases are pages in it rather than a
// second application: `dogfood` is served by `abide dev`, which builds the client into memory — no
// artifact to produce first, and the pages are what is under test rather than the bundle — and
// `mounted` is that same app under a sub-path.
//
