...
* a bun workspace: `packages/abide` is the framework, `packages/harness` is the harness everything is tested and measured with, `packages/dogfood` is the app abide is dogfooded on. see "directory structure" below; import across the seams with the `$server` / `$ui` / `$shared` / `$compiler` tsconfig-path aliases, never with `../`. `./sibling.ts` inside one directory is fine — the rule is about CROSSING a seam, and a `../../` is the tell
* the aliases are `packages/abide`'s OWN. no other package may use them: the harness and the two apps reach the framework by its PUBLIC specifiers — `abide` / `abide/ui` / `abide/server` / `abide/runtime` / `abide/compiler` / `abide/cli` — through the workspace link and abide's own `exports` map, so a broken public entry point fails a typecheck instead of being papered over by an alias. an entry point nothing resolves through is an entry point nothing tests
...
* when changing behavior make sure it is represented in docs/spec.md and in `packages/dogfood/demos` — a demo IS the test and the bench, so there is one place to change, not three. also check comments and remove stale information
...
* `bun run e2e` — the browser gate: playwright, one project per app, specs in each app's `e2e/`. it is for the claims `bun test` cannot reach AT ALL — an `interact` face, a rung RENDERING as opposed to compiling, and a page that produces correct markup while throwing in the console. specs are `*.e2e.ts`, never `*.spec.ts`, because `bun test` claims `.spec.` and would collect them
...
* ./packages/harness - the harness: one `Case` shape with three faces, the assertions, the measurement, and the process spawner. its own package because `harness/measure` must have NO abide in its graph — see "seams and imports"
...
* ./packages/dogfood - the dogfood app: ONE abide app, served by `abide dev` / `abide build && abide start`, whose pages are THREE VIEWS of the same capabilities — written down, running, and priced. there is no second server and no hand-written html per suite
  ...
  * ./pages - what the app SERVES: the directory is the route table. `docs/[callable]` is one public name's ladder, `tests/[suite]/[...rest]` is a capability's cases running, `bench/[suite]` is what they cost, each with an index above it, and the rest is the app's own. TWO vocabularies: docs is keyed by callable and the other two by capability, which is why `app.ts`'s 404 gate carries two lists
  * ./site - what those pages are made of: the case row, the measurement rows, the reference, the section nav, the painted `<pre>`, and the one scanner the panes and the slicing rpc share. FACTS come from the harness and become classes here — see `bench.ts`. the nav lists PAGES and nothing inside one, and `sidebar.ts` carries why: a layout renders before its page, so anything it asks a LOAD about is settled on a server and pending in a browser, and the mismatch rebuilds the page hydration was handed correct markup for
