...
- **A bounded fracture of ADR-0032's uniform model.** "One read, `undefined` everywhere" becomes two read kinds — `$$readCell` (streaming, byte-identical) and `$$readCellBlocking` (throws `SuspenseSignal` on pending) — plus the `SuspenseSignal` class, the `CURRENT_SUSPENSE` slot, and the `suspenseBlock` construct. The fracture is confined to bindings the author opted into blocking on, reuses the `{#try}` recovery machinery rather than reinventing it, leaves `{#await}` blocks untouched, and **closes a latent soundness lie** (today the shadow could type a lifted `await X` as `T` while the runtime returns `undefined` on a cold nav). The type change (D5) is unsound shipped alone, so **D2 + D5 ship as one atomic change** gated on the single predicate.
- **New runtime capability:** the client suspense boundary (sentinel-vs-error discrimination, `CURRENT_SUSPENSE`, the fine-grained per-range withhold + keep-the-watch reveal). Reuses the eager first-run, the warm-seed, and the ADR-0019 recovery loop; no new async-cell primitive.
...
- **`abide check` parity for the D6 thunk-body await-walk.** The bounded descent into an async thunk body must classify identically in the build lowering and the LSP/`abide check` shadow, or a binding could type `T` in the editor and ship streaming (or vice versa). Fold into the D5 parity test.
