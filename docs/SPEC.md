...
| Name | Type Signature | Description |
| --- | --- | --- |
| `s.watch` | `(handler: (value: Stored) => void \| Disposer) => () => void` | Runs effects on value change; returns the way to stop it. See `watch` documentation below. |
...
| Name | Type Signature | Description |
| --- | --- | --- |
| `m.invalidate` | `(pattern?: Partial<Args>) => void` | Marks it stale: the next read is FETCHED FRESH rather than served from what is held. It does not itself hold rendering — whether a template waits is the template's own choice of form (`{await m}` and `{#await m then v}` wait, `{#await m}{:then}` does not). |
| `m.refresh` | `(pattern?: Partial<Args>) => void` | Stale while revalidate: what is held keeps being served, and `m.refreshing()` is true, until the new value lands. |
...
| `tags` | `string[] \| ((args: Args) => string[])` | Arbitrary tags on `memo` that probes can read. The function form receives the memo's `args`. |
...
* reruns `Disposer` each time the `Handler` runs again
...
* So the compiler WARNS on a state read that an `await` dominates inside a tracked body, naming the
...
* A HANDLER THAT CAN BE ADDRESSED OVER HTTP. What it addresses decides what it brings — see `Declarable` — so "it is a memo" is true of the form you generally write and not of the mechanism
* HTTP path is determined by filepath + export name.
* Return type dictates Content-Type
* Browser only sees a wrapped `fetch(rpc.url, { method: rpc.method })` not the whole server module. That wrapper is a module the build GENERATES — one export per declaration, carrying `method`, the mount-relative ADDRESS and `description` and nothing else — rather than the server module shaken down to it, so "no server code shipped" is a property of the build and not an outcome of an optimizer. A module-level side effect in an rpc file cannot reach a browser, because the browser's module was never derived from that file.
* Importing a name from `#server/**` that is NOT an rpc or socket declaration is a COMPILE ERROR from the client graph, never a silent stub; only `#server/rpc/**` and `#server/sockets/**` resolve from `#ui` and `.abide` at all.
...
seal alone, an anonymous caller had nothing to key on, and deciding per declaration whether an answer
...
cross-caller path left to get wrong, and the declaration, the veto and the gate guarding it are
...
* A MEMO BELONGS TO EXACTLY ONE DECLARATION, and handing the same one to two is a BUILD ERROR naming
...
TRANSMITTED"). Every declaration addresses a memo, so three components asking for the same args in one
render are ONE call on both sides and one entry — which is what makes the buffer's single-use rule
enough of a bound.
* Planned or not, errors serialize as `{ name, message, data }` and hydrate as an HttpError `{ name, message, data, status }`
* a schema validation returns 422 with ``TypedHttpError<'ValidationError', readonly Issue[]>``
* A mutation — `POST` / `PUT` / `PATCH` / `DELETE` — compares its `Origin` against `APP_URL`'s origin before the handler runs, and a mismatch is 403. `Origin` is sent on every cross-origin request, form posts included, and cannot be set by script, so the gate holds independently of cookie policy; a declared `crossOrigin` allow-list replaces the same-origin comparison for that endpoint. `GET` needs no gate because it is a read, and `DELETE` is unreachable by navigation because a navigation is always GET. A mutation arriving with NO `Origin` at all is ALLOWED: CSRF needs a browser's ambient authority and a browser always sends one, so an absent header is a non-browser caller with no ambient cookie to abuse. Refusing those callers is an app's rung, which is what middleware is for
...
preflight is answered OUTSIDE the onion, for the reason `/__abide/health` is: it carries no credentials,
...
this declaration's method, `access-control-allow-headers` echoed from `access-control-request-headers`,
...
A path is owned by exactly one declaration, and a collision is a BUILD ERROR naming both files —
...
* `JsonSchema` is the native form — what a declaration MEANS, and the only one publishable to a tool definition or OpenAPI operation.
...
* `GET /__abide/health` returns `Health` over HTTP. Answered OUTSIDE the middleware onion, because the load balancer asking presents no credentials — so it is unauthenticated, and whatever `onHealth` merges is public.
...
| `error` | `WireError \| undefined` | The app's resolver having failed. Present only then, and a caller carrying one is never `authenticated`. |

`claims` is the app's half, and the wire document is the baseline with it spread alongside — so
`principal.claims()` reads what `onPrincipal` returned and nothing of the baseline.
...
| Name | Type Signature | Description |
| --- | --- | --- |
| `schema` | `Schema<Extra>` | The same `Schema` a transport declaration takes, checked LAST over the whole document. It normalises as well as refuses. |
...
| `abide:watch` | `warning` | A `watch` handler or `Disposer` that threw, and the watch it stopped |
| `abide:hydrate` / `abide:navigate` | `warning` | The browser lane: a mismatched slot rebuilt, a route whose chunk did not arrive |
...
* Channels ask the `log.enabled` before doing any work of their own: closed, none of them
* `GET /__abide/logs` tailed over HTTP as jsonl |
...
Inside a `.abide` file a state is read and written by NAME: `foo` where a `.ts` file writes `foo()`,
...
| `{await expr}` | await in an expression means block rendering until resolved. Over a STREAM that means until the stream CLOSES, the same way it means until a value settles — one rule, and a source that never closes holds forever, which is the author's to resolve rather than abide's to special-case. |
| `{raw(...)}` | Raw HTML |
...
A SINK is an addressable slot in the output stream that a value still in flight fills later. It is
...
* A fill has two modes. FILL writes the value into the slot. REPLACE swaps the enclosing boundary's
REGION, which is what a failure needs: a `{#try}` body cannot be repaired by filling one slot inside
it, the markup around that slot having already flushed. `error.abide` is the outermost boundary and
...
buffered, and no round trip is paid — for a declaration over a MEMO, which is one call on both sides;
...
what `abide:hydrate`'s "a mismatched slot rebuilt" is reporting.
* After first hydration the client does all rendering after document request onion returns ok. Do what rendering can be done while waiting for document request onion. If there is no middleware, document request does not need to run.
...
| `ABIDE_RPC_TIMEOUT` | `number` | Default ms a call may go without progress (default `Infinity`); a declaration's `timeout` is the real knob. Per chunk on a handler that yields. |
...
| `ABIDE_LOGS` | `boolean` | Opts IN to `GET /__abide/logs`. Default closed → 404. The feed is dispatched THROUGH the app's `middleware`, so an app's auth rung governs it like any route. |
| `ABIDE_MAX_LOG_BUFFER_COUNT` | `number` | Ring size in records for that feed (default `500`). |
...
| `onError` | `(fn: (error: unknown) => unknown) => () => void` | Runs on an UNEXPECTED error in this scope: a request that threw, and a `watch` handler or `Disposer` that threw. Scope-shaped rather than request-shaped, since a watch outlives the read that started it |
| `onConfig` | `(fn: ConfigDefaults \| null, options?: { schema }) => () => void` | The app's config DEFAULTS, merged UNDER the environment. Synchronous, and the only one that fails hard |
...
An app's own route answers with the SAME helpers a transport declaration does — `page`, `json`,
...
| `abide start [--port <n>]` | Boot the app's build from `abide build` |
