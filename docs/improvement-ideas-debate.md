...
Ten haiku agents each proposed one unique improvement to belte (one duplicate — a second
...
| 6 | **Cache lifecycle diagnostics** — log registrations, hits/misses, invalidations, TTL expiry, inline-vs-streaming decisions | Adopt `DEBUG=belte:cache` channel; reject a public `cache.debug()` callback (API growth) | Agree on no public callback, but the env flag alone misses the browser half: pair server-side `DEBUG=belte:cache` with a client `localStorage` toggle (`belte-debug=cache`) wired to the same diagnostic channel |
...
| 8 | **`createForm()` optimistic-mutation form primitive** — field state, optimistic updates, rollback | Reject for core: large opinionated surface and parallel reactivity machinery; existing idiom (verbs + `pending()` + invalidate + runes) composes; revisit as `@belte/forms` only with demand | Agree on keeping it out of core, but carve out a thin `errorMap()` helper (server validation error → per-field message map) in `@belte/forms` — real repetitive glue, no validation-timing opinions, no reactivity machinery |
| 9 | **Persistent cache adapter (Bun SQLite)** — durable `CacheStore` surviving restarts | Defer: persistence imposes serializability on cached values and changes settle semantics; prototype out-of-core as `@belte/cache-sqlite` first | Agree, sharpened from "defer" to "build now, out of core": ship `@belte/cache-sqlite` in the monorepo as explicit opt-in (per key/store), loud errors on non-serializable values, zero changes to core `cache()` until real usage patterns emerge |
| 10 | **`__belte/playground` interactive surface explorer** — live schema forms for routes/RPCs/sockets/MCP | Adopt dev-only, built into the framework as a UI over already-generated surface metadata | Agree on the value but not the location: ship as a separate opt-in `@belte/explorer` package consuming the generated metadata — schema-driven UI for sockets/MCP is substantial ongoing maintenance that doesn't belong in a size-conscious core |
...
- **Relocated (1):** idea 10 — same feature, but as `@belte/explorer` instead of in-core.

A recurring theme across the debates: belte's own stated values were used against the
