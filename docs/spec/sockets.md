...
- **`route().kind` is a discriminated union** the framework populates from routing:
  ...
  - `'socket-connect'` — WS upgrade to the mux (no specific socket named yet); this is the kind the
    GLOBAL chain sees when it runs at the upgrade.
  - `'socket-subscribe'` / `'socket-publish'` — HTTP-face ops; `name` = socket name.
  - ~~`'stream'` — §12 streaming read (if distinguished)~~ — **REMOVED (2026-07-29).** Never produced,
    so branching on it type-checked and could never run. A streaming rpc reports `'rpc'`, which is the
    honest answer: same handler, same name, same authorization.
