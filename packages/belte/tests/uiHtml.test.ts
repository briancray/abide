...
function ssr(source: string, model: unknown): SsrRender {
    ...
    return new Function(...names, compileSSR(source))(...Object.values(RUNTIME), model) as SsrRender
}
...
describe('html`` raw markup via {expr}', () => {
    test('the html tag brands a value (plain call + tagged)', () => {
        ...
        expect(out).toBe('<div><!--belte:html--><b>hi</b><!--/belte:html--></div>')
    ...
    test('tagged form concatenates parts verbatim', () => {
        ...
        expect(out).toBe('<div><!--belte:html--><b>x</b><!--/belte:html--></div>')
    })
