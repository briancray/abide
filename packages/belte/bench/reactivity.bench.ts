...
Reactivity write-path benchmark. Run with the Svelte client loader preloaded:
  ...
  bun --preload ./packages/belte/tests/support/sveltePreload.ts \
      packages/belte/bench/reactivity.bench.ts

Workload (same for all): a list of N items, M times "update one item's field and
read it back". Three contenders:

  - belte cell   — `doc.cell(path)`, the stable accessor the compiler emits:
                   path resolved once, the hot loop is string-free. Apples to
                   apples with Svelte's compiled "hold the signal reference".
  - belte path   — `doc.replace(path)` / `doc.read(path)` building the path
...
function belteCellBench(itemCount: number, updates: number): BenchResult {
...
function beltePathBench(itemCount: number, updates: number): BenchResult {
    const document = doc({ items: Array.from({ length: itemCount }, (_, index) => ({ n: index })) })
    let sink = 0
    const updateStart = performance.now()
    for (let update = 0; update < updates; update += 1) {
...
belteCellBench(200, 5_000)
beltePathBench(200, 5_000)
svelteCellBench(200, 5_000)

console.log(`\n1) head-to-head: ${HEAD_TO_HEAD_ITEMS}-item list, ${UPDATES} update+read\n`)
report('belte cell', belteCellBench(HEAD_TO_HEAD_ITEMS, UPDATES), UPDATES)
report('svelte $state', svelteCellBench(HEAD_TO_HEAD_ITEMS, UPDATES), UPDATES)
report('belte path', beltePathBench(HEAD_TO_HEAD_ITEMS, UPDATES), UPDATES)

console.log(`\n2) belte cell scaling across list width (${UPDATES} update+read)\n`)
for (const width of [100, 1_000, 5_000, 20_000]) {
    report(`belte width=${width}`, belteCellBench(width, UPDATES), UPDATES)
}
