...
const DOGFOOD_ROOT = `${import.meta.dir}/..`

/**
 * The dogfood app's own APP, which is the standard SPEC names — its pages, what answers them, and what
 * they are made of. Not `demos`, `test` or `fixtures`: those exercise the framework rather than use
 * it, and counting them would make every export look typed by somebody.
 */
const AUTHORING_LANES = '{pages,server,site}/**/*.{ts,abide}'

/** The `.abide` half of the same lanes, whose emitted header is authored on the file's behalf. */
const COMPILED_LANES = '{pages,site}/**/*.abide'
...
const UNUSED_BY_THE_APP: Record<string, string> = {
    ...
    url: '`pages/layout.abide:41` builds `/${other}/${name}` with a template literal, which is this call’s whole job.',
}
