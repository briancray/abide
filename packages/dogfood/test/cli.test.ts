...
// number, so a process around it would test the process; and it cannot go in `packages/dogfood/demos`
...
test('`run` puts a script under the runtime, and hands it its own arguments', async () => {
    ...
    await Bun.write(
        ...
        `import Card from '${import.meta.dir}/../demos/fixtures/card.abide'\n` +
...
test('`check` reports a `.abide` type error on the `.abide` line', async () => {
    ...
    const checked = await abide(['check', '.'], { cwd: `${import.meta.dir}/../types/invalid` })
...
test('`repl` imports a `.abide` file, resolved against where the prompt was opened', async () => {
    ...
    const said = await replies(
        ...
        `${import.meta.dir}/../demos/fixtures`,
    )
