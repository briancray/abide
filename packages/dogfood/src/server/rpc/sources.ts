...
import { META, type SuiteName } from '#shared/demos/SUITES.ts'
import { type Face, type Span, scan, sliceOf } from '#ui/lib/code.ts'
...
function fileFor(suite: string): URL | null {
    ...
    // RESOLVED through the seam rather than counted in `../` from here. The suites are in the shared
    // seam and this handler is in the server one, so what sits between them is a fact about the app's
    // layout and not about this file's depth — `#shared/demos/…` is that fact, and it is the same
...
/**
 * One case's `run`, `server` or `interact`, exactly as written.
 *
