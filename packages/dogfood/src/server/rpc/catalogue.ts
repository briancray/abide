// The `data` use case's supply — one endpoint, because of where this file sits.
//
...
//
// `Data.abide` is a browser view, so this module reaches the client lane as its ADDRESS and none of
// this text — `buildCatalogue` and the ~10,000 objects it makes stay on this side of `server/rpc/`.

import { GET } from 'abide/server'
import { buildCatalogue, type CatalogueEntry } from '#shared/demos/usecases/catalogue.ts'
