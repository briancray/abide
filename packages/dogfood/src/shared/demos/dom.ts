...
// the site IS an abide app now — `#ui/lib/CaseRow.abide` renders every card these fill in. What stays
...
export function stage(host: HTMLElement, label = 'live'): HTMLElement {
    ...
    // `label` is the small-caps ramp, the same class the tables reach through `#ui/lib/table.ts`.
