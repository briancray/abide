...
import { type Config, config, onConfig } from 'abide/server'
// The accessor behind `APP_NAME`, which is not public: an app reads the FIELD. Imported here to
// assert the two agree, which is the whole reason there is only one of them on the surface.
import { appName } from 'abide/server/internal'
import { boot } from 'abide/server/internal'
    ...
    cases: [
        {
            ...
            run({ is }) {
                ...
                // Three CONCLUSIONS that are fields anyway — the one place the variables-only rule
                // is broken, and broken deliberately: an app asks what it is called far more often
                // than it asks which variable said so. `NODE_ENV`'s conclusion is NOT one of them,
                // because `isProduction()` is drawn internally by `identity.ts` and never published.
                is('the app has a name', settings.APP_NAME.length > 0, true)
                is('a version, empty rather than absent', typeof settings.APP_VERSION, 'string')
                is('and somewhere to write', settings.APP_DATA_DIR.length > 0, true)
                // The variable is still the variable, and the field is it RESOLVED — so a document
                // that published one and honoured the other would show up right here.
                is(
                    'ABIDE_APP_NAME is what an operator sets; APP_NAME is what it resolved to',
                    settings.ABIDE_APP_NAME === null || settings.ABIDE_APP_NAME === settings.APP_NAME,
                    true,
                )
            },
        },

        {
            ...
            async run({ is }) {
                const off = onConfig((env) => ({
                    ...
                    CHECKOUT_URL: `${env.APP_NAME}/checkout`,
                }))
                ...
                is('computed off a field', settings.CHECKOUT_URL, `${settings.APP_NAME}/checkout`)
        ...
        {
            title: 'a derived fact IS a field — and so is the variable under it',
            note: 'A name, a version and a data directory are CONCLUSIONS rather than variables, and they are published as fields anyway — the one place the variables-only rule is broken, and broken because an app asks what it is called far more often than it asks which variable said so. `APP_NAME` is `ABIDE_APP_NAME`, else the nearest package.json, which knows more about what this app is called than a fallback written inside it; `APP_DATA_DIR` is `ABIDE_DATA_DIR` resolved against the directory the PLATFORM chose. What makes two spellings safe is that there are not two ANSWERS: the field is computed inside `resolve()` from the same function that used to be the accessor, so a document publishing one value while the app honoured another is unreachable rather than merely discouraged. The variable is still the variable and the field is it resolved — so defaulting `ABIDE_APP_NAME` moves `APP_NAME`, and the climb underneath is not a knob and has nothing to default.',
            run({ is }) {
                const climbed = config().APP_NAME
                const off = onConfig(() => ({ ABIDE_APP_NAME: 'renamed' }))

                is('the variable takes the app’s default', config().ABIDE_APP_NAME, 'renamed')
                // The half that makes the pair safe: a default the conclusion did not follow would be
                // a document naming this app one thing while every log line named it another. This is
                // why the three are resolved a SECOND time after the hook's layer is merged.
                is('and the field over it followed', config().APP_NAME, 'renamed')
                is('as did the accessor abide itself calls', appName(), 'renamed')

                off()
                // Nothing declared, so it is the climb again rather than a floor written into the
                // table — a conclusion has no floor of its own to fall back to.
                is('off again is what the package.json says', config().APP_NAME, climbed)
                is('and the three are on the document', 'APP_NAME' in config(), true)
                is('…including the one only a filesystem can answer', config().APP_DATA_DIR.length > 0, true)
            },
        },
...

        {
            ...
            async run({ is }) {
                ...
                let resolves = 0
                const logging = onConfig((env) => {
                    resolves++
                    log(`resolving as ${env.APP_NAME}`)
                    return { ABIDE_APP_NAME: 'renamed-by-the-hook' }
                })
                is('the field over the variable it set', config().APP_NAME, 'renamed-by-the-hook')
