...
// is what it means. The last three change WHERE the state lives — through a transform, out to module
// scope, and finally keyed by a name — so each is the same state asked one more question.
...
import Seven from './7-a-state-outlives-a-render.abide'
import SEVEN from './7-a-state-outlives-a-render.abide?source'
import Eight from './8-one-state-by-name.abide'
import EIGHT from './8-one-state-by-name.abide?source'
...
export const LADDER: Example[] = [
    ...
    {
        adds: 'derive from it — using a state in an expression reads it',
        note:
            'Naming the state alone hands over the STATE; using it in an expression READS it. Which one ' +
    ...
    },
    {
        ...
        note:
            ...
            'wants the whole stream and asks the state gets only the last thing to land.',
        of: ['state'],
        source: FOUR,
        view: Four,
    },
    ...
    },
    {
        ...
        note:
            'It runs on the `initial` as well as on every write, so the state cannot hold a value the ' +
    ...
    {
        adds: 'module scope, so one state outlives any single render and is shared per CALLER',
        note:
    ...
    {
        adds: 'one state by NAME rather than by reference, which module scope cannot do',
        note:
            'Two files that never import each other reach the same state, and the FIRST call decides its ' +
