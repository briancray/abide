    ...
    cases: [
        {
            ...
            interact({ host, log }) {
                ...
                mount(
                    ...
                    () => html`
                        <p class="text-ink">
                            <span class="text-pencil">name</span> ${() => name()} ·
                            <span class="text-pencil">count</span> ${() => count()}
                        </p>
                    `,
                )
                host.append(
...

        {
            ...
            async run({ is }) {
                ...
                // in the same tick, so a global spy counted 2 in a browser and 1 under `bun test`,
                // where there is no page. An own property shadows the prototype method for this node
                // alone, which is the only receiver this claim is about.
                const node = host.querySelector('i') as Element
                const spy = countCalls(node, 'setAttribute')
                other.set(1) // moves only data-n
                await tick()
                spy.restore()

                is('data-n', node.getAttribute('data-n'), '1')
...

        {
            ...
            async run({ is }) {
                for (const [label, reorder, expected] of [
                    ...
                    // The LIST, not `Node.prototype`: the page this runs inside moves nodes of its own
                    // — a status badge, a console line — and a global spy attributes them here. It
                    // counted right in a browser only because nothing else happened to move in the
                    // same tick, which is a gate that holds by luck.
                    const list = host.querySelector('ul') as Node
                    const spy = countCalls(list, 'insertBefore')
                    rows.set(reorder)
                    await tick()
                    spy.restore()
...

        {
            ...
            interact({ host, log }) {
                ...
                const value = state(1)
                const out = stage(host)
                const paneA = (n: number): TemplateResult => html`<p class="text-verdigris">pane A · ${n}</p>`
                const paneB = (n: number): TemplateResult => html`<p class="text-brass">pane B · ${n}</p>`
...

        {
            ...
            async run({ is, log }) {
                ...
                let ran = 0
                const host = container()
                // The compiler's own emit for a row with a conditional class — see the `complex` use
                // case in `#shared/demos/usecases/`, which is where these three ops come from.
                mount(
                    host,
                    () =>
                        html`<ul>${() =>
...

        {
            ...
            interact({ host, log }) {
                const n = state(0)
                const out = stage(host)
                const mounted = mount(out, () => html`<p class="text-ink">n = ${() => n()}</p>`)
                let disposed = false
                host.append(
                    row(
...

        {
            ...
            async run({ is }) {
                ...
                const quote = memo(async ({ symbol }: { symbol: string }) => {
                    // A real delay: on a resolved promise there is no window to invalidate INSIDE.
                    await sleep(20)
                    return `${symbol} @ ${++loads}`
                })
                const host = scratch(
                    () =>
                        html`${() =>
                            quote({ symbol: 'ABC' }).pending()
                                ? html`<p>loading…</p>`
                                : html`<p>${() => quote({ symbol: 'ABC' })()}</p>`}`,
                )
                await sleep(40)
                is('the first load', host.textContent, 'ABC @ 1')

                quote({ symbol: 'ABC' }).invalidate()
                await tick()
                is('invalidating a settled slot shows the pending arm', host.textContent, 'loading…')

                quote({ symbol: 'ABC' }).invalidate()
                await tick()
                is('…and so does invalidating a loading one', host.textContent, 'loading…')

                await sleep(40)
                is('the third load lands', host.textContent, 'ABC @ 3')
                host.remove()
            },
        },
