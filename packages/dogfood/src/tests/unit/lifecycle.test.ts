...
// deliberate/unexpected split — ARE card-showable and live in `#shared/demos/lifecycle.ts`.
//
...
import { type Reading, reading } from 'harness/spawn'
import { APP_ROOT as DOGFOOD_ROOT } from '#tests/PATHS.ts'
...
function start(mode?: string, declared?: Record<string, string | undefined>): Reading {
    return reading(['bun', APP, ...(mode === undefined ? [] : [mode])], {
        // The dogfood package rather than the repo root, because `name` is the NEAREST package.json
...
// `#shared/demos/config.ts` owns the resolution ORDER and covers more of it than this does — coercion, the
...
test('config layers a real environment over the app’s defaults over abide’s floor', async () => {
    ...
    expect(JSON.parse(await defaulted.until('')) as unknown).toEqual({ port: 8080, tag: 'dogfood-tag' })
    ...
    expect(JSON.parse(await declared.until('')) as unknown).toEqual({ port: 9123, tag: 'dogfood-tag' })
