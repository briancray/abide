...
// onion, the render walk, the route table — is already in `#shared/demos/lifecycle.ts` and `#shared/demos/server.ts`.
//
...
/** What the lane compiled to, which is the only name that is not a guess. */
let entry: string
...
test('a route’s own chunks are named in its head, so the entry is not what discovers them', async () => {
    ...
    const own = graph.modules['src/ui/pages/users/[id]/page.abide'] as string
    const layout = graph.modules['src/ui/pages/users/layout.abide'] as string
...
test('a file in the public directory is served at its own name, in front of the pages', async () => {
    // The address IS the filename: `src/ui/public/robots.txt` answers at `/robots.txt`, and nothing in
    // this app names either the directory or the file. That is the whole difference from the client
    // bundle — those names carry a content hash and live under a reserved prefix, where these are
    // whatever the author typed and sit at the origin root.
    const robots = await fetch(`${app.base}robots.txt`)
    expect(robots.status).toBe(200)
    expect(await robots.text()).toContain('User-agent: *')
    // The type comes off the extension rather than from a table this had to write.
    expect(robots.headers.get('content-type')).toContain('text/plain')
    // Cached BRIEFLY, not forever: the name does not change when the bytes do, so a year of
    // `immutable` would be a stale favicon nobody can flush.
    ...
    // A nested one, and a type that is not text — the scan is `**/*`, so the tree is the address space.
    const icon = await fetch(`${app.base}favicon.svg`)
    expect(icon.status).toBe(200)
    expect(icon.headers.get('content-type')).toContain('image/svg+xml')

    // And the layer does not swallow what it does not hold: a name the directory has no file for is
    // the router's 404, not an empty 200. This is the half that would still pass with the lookup
    // replaced by "serve anything", which is why it is here rather than assumed.
    const missing = await fetch(`${app.base}not-a-public-file.txt`)
    expect(missing.status).toBe(404)
})
...
test('a handler serves because of where it SITS, not because something imported it', async () => {
    ...
    const source = await Bun.file(`${ROOT}/src/server/app.ts`).text()
...
test('the bundle is served immutable, precompressed, and in front of the onion', async () => {
    ...
    // ELISION is asserted where the stub now lands, in `#ui/pages/users/[id]`'s chunk; see `build.test.ts`.
...
test('the page is served in the app’s own app.html, in the slot', async () => {
    ...
    expect(markup).toContain('<title>abide dogfood</title>')
    ...
    // A comment that TALKS about the slot is not the slot — the document explaining itself is the
    // first document anybody writes, and this one does.
    expect(markup).toContain('`<slot></slot>` is where a page renders')

    // And the head names nothing of abide's. The script is APPENDED from the manifest, so the file
    // carries no path to a lane the app never wrote and no hash it would have to be rebuilt to know.
    expect(markup).toContain(`<script type="module" src="${CLIENT_ROUTE}${entry}">`)
    expect(markup).not.toContain('client.ts')
})

test('a stylesheet a page imported is built, linked and served', async () => {
    const markup = await (await fetch(app.base)).text()

    // `#ui/pages/layout.abide` writes `import '#ui/app.css'` and `app.html` names no stylesheet at all:
...
test('what makes a directory an app: nothing refuses, pages with no build refuse, endpoints alone come up', async () => {
    ...
    try {
        ...
        // so every `<script>` they emit would 404. Asked of the pages directory and not of a client entry,
        ...
        await Bun.write(`${empty}/src/server/app.ts`, 'export default (): undefined => undefined\n')
        await Bun.write(`${empty}/src/ui/pages/page.abide`, '<h1>home</h1>\n')
        ...
        await rm(`${empty}/src/server/app.ts`)
        await mkdir(`${empty}/src/server/rpc`, { recursive: true })
        await Bun.write(
