...
const FIXTURE = `${TYPES}/checker/server/rpc/computed.ts`
...
test('the app’s own endpoints are covered, and the addresses are the same ones', async () => {
    const app = await deriveShapes({ roots: [`${APP_ROOT}/`] })
...
const BOTH = `${TYPES}/checker/server/rpc/both.ts`
const CHANNELS = `${TYPES}/checker/server/sockets/channels.ts`
