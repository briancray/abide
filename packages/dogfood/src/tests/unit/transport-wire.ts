...
// put back by the preload; see the harness's `internal/happydom.ts` for where that line is drawn and why.)
...
//
// Run it directly to watch it: `bun packages/dogfood/src/tests/unit/transport-wire.ts`.

import { abidePlugin } from 'abide/compiler/plugin'
import { remote, remoteSocket } from 'abide/runtime/transport'
import { dispatch, registered, websocket } from 'abide/server/internal'
import { until } from 'harness'
import { plugin } from 'bun'
import { SERVER } from '#tests/PATHS.ts'
