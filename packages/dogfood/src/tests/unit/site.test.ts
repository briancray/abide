...
// browser's. What a card does once it HAS one is `#shared/demos/*.ts`, which `demos.test.ts` runs.
...
test('the nav is the four sections, and a capability page links to its other views', async () => {
    ...
    const layout = (await import('#ui/pages/layout.abide')).default
...
test('scanning a demo file loses nothing and reorders nothing', async () => {
    for (const suite of SUITES) {
        const source = await Bun.file(`${DEMOS}/${suite.name}.ts`).text()
