// A LOCAL that happens to share a state's name, which the compiler was rewriting into a read.
//
...
// on a value that was never a state.
//
...
//     the name it binds
//
// The last assertion is the one that catches over-fixing: an actual state read must still be a read.
...
describe('a local named like a state is NOT a read', () => {
    const shadows: [string, string][] = [
        ...
        [
            'destructured',
            'function f(input: { showing: number[] }): number {\n    const { showing } = input\n    return showing.length\n}',
        ],
        [
            'destructured and renamed',
            'function f(input: { a: number[] }): number {\n    const { a: showing } = input\n    return showing.length\n}',
        ],
        [
            'a for-of binding',
    ...
    for (const [what, body] of shadows) {
        test(what, () => {
            ...
            const inBody = code.slice(
                code.indexOf('function f') === -1 ? code.indexOf('const f') : code.indexOf('function f'),
            )
    ...
    test('a parameter stops shadowing at the end of its function', () => {
        ...
        // below, which is a real state read, came out as a bare identifier.
        const code = emitted(
            'function f(showing: number[]): number {\n    return showing.length\n}\nfunction g(): number {\n    return showing\n}',
        )
