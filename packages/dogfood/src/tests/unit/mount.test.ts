...
// The half a card CAN make is in `#shared/demos/routing.ts` — `url`, `navigate` and the table, which is where
...
// `demo/…`, and its `app.html` names no address at all — one environment variable moved every
...
test('the document points the browser at the mounted bundle', async () => {
    ...
    // `app.html` names no script at all and knows nothing about any of this. The tag is appended, and
    // what the lane compiled to is read from the manifest rather than guessed, as `start.test.ts` does.
