// A booted app, spawned, for `#tests/unit/lifecycle.test.ts`.
//
...
// it, which under `bun test` is the runner. The card-showable half is `#shared/demos/lifecycle.ts`.
//
...
import { config, json, onConfig, onStart, onStop } from 'abide/server'
import { boot, handle, shutdown, websocket } from 'abide/server/internal'
...
if (mode === 'config') {
    ...
    onConfig((env) => ({ PORT: 8080, TAG: `${env.APP_NAME}-tag` }))
