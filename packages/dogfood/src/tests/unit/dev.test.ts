...
test('the client is built into memory, with nothing on disk to have built first', async () => {
    ...
    // that file is `.abide/client.entry.ts` — generated from `#ui/pages/`, for every app. A document
    // writes no script at all; `shell.ts` appends this one off the manifest.
    ...
    //
    // Followed into the CHUNK rather than read off the entry. The lane is generated from `#ui/pages/`, so
    // the module importing the endpoint is `#ui/pages/users/[id]/page.abide` — and a page being its own
...
test('a change restarts the app, and the socket a browser holds is what notices', async () => {
    ...
    const now = new Date()
    await utimes(`${ROOT}/src/server/app.ts`, now, now)
