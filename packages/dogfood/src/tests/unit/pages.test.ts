...
// card CAN show is in `#shared/demos/routing.ts`, driven by a hand-written table — which is the same shape
// this produces, so nothing here is a second implementation of anything.
//
// The pages under `#ui/pages` are real `.abide` files, imported through the compiler
...
import { PAGES } from '#tests/PATHS.ts'

const HERE = new URL(`file://${PAGES}/`)
...
test('a directory is a pattern and a filename is a kind', async () => {
    ...
    // rather than only in `#shared/demos/routing.ts`: `/docs` and `/docs/state` both exist, and the literal
...
test('every layout above a page wraps it, outermost first', async () => {
    ...
    // and `#shared/demos/routing.ts` drives a table with a single layout, so this is the only line in the
