...
// isomorphic (`isolate`, per-caller caches, `{ global }`) lives in `#shared/demos/scope.ts` where it belongs.
//
...
import { withEnv, writeEnv } from '#shared/demos/env.ts'
 ...
 *
 * A BOUNDED ladder, and it has to be: `microtasks()` in `harness/measure` re-schedules itself for
...
test('a served body does not await a chunk its source already had', async () => {
    ...
    // is why no existing case could see it: `jsonl([1, 2, 3]).text()` in `#shared/demos/transport.ts` runs
...
test('a keyed memo is per-request under serve', async () => {
    ...
    // called, so `serve` and `isolate` are one branch and `#shared/demos/scope.ts` already owns it.)
    let bodyRuns = 0
...
// than in `#shared/demos/routing.ts` for the reason at the top of this file — the demos drive a route with an
...
// isomorphic — the anonymous floor, the two writers refusing off a client — is in `#shared/demos/identity.ts`.
...
test('a stale disposer does not reach past itself and clear the registration that replaced it', async () => {
    ...
    // `#shared/demos/identity.ts` walks this sequence but asserts `typeof second === 'function'`, which is
...
// The status, the `no-store` header and the anonymous floor are `#shared/demos/identity.ts`'s — it reaches
...
/**
 * A memo the handler reads and the BODY reads again, counted.
 *
 * At module scope on purpose: that is the shape `serve` exists for — one declaration, a cache per
 * caller — and it is the shape whose cost the hold decides.
 */
let rowRuns = 0
const row = memo(() => {
    rowRuns++
...
async function* streamedPage(): AsyncGenerator<string> {
    yield '<p>'
...
test('page() takes a generator, and holding it is what stops the cache being rebuilt', async () => {
    // `page(render(view))` is the composition this is here for: `render` IS an async generator, so
    // before `page` accepted one an app had to reach for `toStream` — and a body handed straight to
    // `new Response` works, which is exactly the problem. Bun accepts an async iterable, so the wrong
    // implementation produces the RIGHT MARKUP with the request scope already dropped under it.
    //
    // So this asserts the WORK. Without the hold the body's read of `row` misses a cache that is gone
    // and runs the memo a second time: right answer, twice the work, nothing to say so. Verified by
    // handing the generator to `new Response` unheld — 1 becomes 2.
    rowRuns = 0
    const answered = await serve(new Request('https://example.test/page'), () => {
        row()
        const handler = GET(() => page(streamedPage()))
        return handler.raw({})
    })
    expect(answered.headers.get('content-type')).toContain('text/html')
    // The body is read AFTER the handler returned, which is the whole point.
    expect(await answered.text()).toBe('<p>x</p>')
    expect(rowRuns).toBe(1)
})

test('a handler’s own Response is passed through, not encoded into an envelope', async () => {
    // `respond` settles the handle and JSON-encodes what it got. A handler that built a `Response`
    // has already said what its answer IS, so it goes back as it was — with the wire's headers merged
    // UNDER what the handler set, so `content-type` survives and `traceresponse` still arrives.
    const rendered = GET(() => page('<h1>ada</h1>'))
    const answered = await rendered.raw({})
    expect(answered.headers.get('content-type')).toContain('text/html')
    expect(await answered.text()).toBe('<h1>ada</h1>')

    // The other arm, which is what says the passthrough did not swallow the ordinary case.
    const valued = GET(() => ({ id: 1 }))
    const envelope = await valued.raw({})
    expect(envelope.headers.get('content-type')).toContain('application/json')
    expect(await envelope.text()).toBe('{"id":1}')
})
