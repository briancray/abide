    ...
    cases: [
...

        {
            ...
            interact({ host, log }) {
                ...
                const panes = rooms.map((room) => {
                    const pane = el('p', 'font-mono text-xs text-verdigris min-h-5')
                    out.append(pane)
                    reader(() => {
...

        {
            ...
            interact({ host }) {
                ...
                const out = stage(host)
                const current = el('p', 'text-3xl font-semibold text-ink tabular-nums', '—')
                const mean = el('p', 'text-sm text-ink-soft', 'mean of the last 8: —')
                const spark = el('p', 'font-mono text-xs text-verdigris mt-1')
