...
import type { BenchHandle } from 'harness'
import { expect, test } from 'harness/e2e'
import { type EngineWork, engine } from 'harness/engine'
...
test('what each arm RETAINS, and what it made the engine do', async ({ page }) => {
    ...
    // `{ collect: true }` is the whole reason this lane exists. Without a forced collection `nodes`
    // counts whatever has not been swept yet, which drifts on its own between two reads of an idle
    // page — so a delta would be noise with a leak somewhere inside it rather than the leak.
    //
    // The counters themselves are `harness/engine`'s now. This file used to reach for CDP directly and
    // was one of the two places the same eight lines lived.
    const reading = await engine(page, { collect: true })

    const per = (n: number): number => n / OPS
    ...
    for (const suite of SUITES) {
        ...
        for (const row of listed) {
            ...
            for (const arm of row.arms) {
                ...
                const settling = await reading.around(() => runArm(row.title, arm))
                const steady = await reading.around(() => runArm(row.title, arm))

                const leak = (of: keyof EngineWork): number => per((steady[of] as number) ?? 0)
                const resident = (of: keyof EngineWork): number =>
                    ((settling[of] as number) ?? 0) - ((steady[of] as number) ?? 0)
                console.log(
                    `    ${arm.slice(0, 50).padEnd(50)}${cell(leak('nodes'))}${cell(leak('heapBytes'), 0)}${cell(resident('nodes'), 0)}${cell(leak('recalcStyle'), 2)}${cell(leak('layout'), 2)}`,
                )
                layouts += leak('layout') + leak('recalcStyle')
