...
export type Face = 'run' | 'interact'

/**
 * The text of one case's `run` or `interact`, exactly as it is written in `source`.
 *
 * Found by the case's TITLE, which is the one thing about a case that is unique within its suite and
 * is already how the page names it. From there it is the next `run`/`interact` at the depth the title
 * itself sits at — the case's own object — so a `run` nested inside an earlier case's body cannot be
 * mistaken for the next case's.
 *
