...
test('a row is what, abide, by hand and the difference — and the evidence is behind it', async ({
    ...
    const p50 = nanos(await first.locator('div.font-mono').first().innerText())
    ...
    // CALIBRATED count rather than one batch per pass is `kit.test.ts`'s to assert, and it does.
