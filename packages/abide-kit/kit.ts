// The kit's front door — one shape for a demonstration, a test and a bench.
//
...
//
// This entry point imports `abide`; `abide-kit/measure` deliberately does not, and the numbers live
// there for that reason. `abide-kit/spawn` is the third, and is bun-only.
...
export interface Suite {
    ...
    blurb: string
    /**
     * The ladder, smallest rung first. A suite without one is a capability nobody can copy.
     *
     * ORDER IS THE CONTENT: each rung is the one before it plus one new thing, so a rung inserted out
     * of order introduces something a reader has not been shown yet.
     */
    examples?: Example[]
    cases: Case[]
}
...
export function suite(spec: Suite): Suite {
    ...
    // A one-rung ladder is a single example wearing an array, and every capability here has more than
    // one thing to say. Caught at declaration rather than by a reader noticing the page is thin.
    if (spec.examples !== undefined && spec.examples.length < 2) {
        throw new Error(`abide: suite "${spec.name}" has a ladder with one rung`)
    }
    return spec
}
