...
describe('runWithRequestScope', () => {
    ...
    test('dev mode (ABIDE_DEV=1) opts the framework 500 into the full stack', async () => {
        const previous = Bun.env.ABIDE_DEV
        Bun.env.ABIDE_DEV = '1'
        try {
            const response = await runWithRequestScope(
                new Request('https://test.local/'),
            ...
            if (previous === undefined) {
                delete Bun.env.ABIDE_DEV
            } else {
                Bun.env.ABIDE_DEV = previous
            }
        }
    })
