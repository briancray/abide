...
test('ABIDE_DATA_DIR overrides the platform default as-is, no program name appended', () => {
    const previous = process.env.ABIDE_DATA_DIR
    process.env.ABIDE_DATA_DIR = '/tmp/abide-devdata'
    try {
        expect(appDataDir('chill')).toBe('/tmp/abide-devdata')
    } finally {
        if (previous === undefined) {
            delete process.env.ABIDE_DATA_DIR
        } else {
            process.env.ABIDE_DATA_DIR = previous
        }
    }
})
