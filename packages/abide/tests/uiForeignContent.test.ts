...
<svg viewBox="0 0 24 24"><template await={load()}><circle cx="1"/><template then="v"><circle r={v}/></template></template></svg>
`
...
describe('foreign content — imperative path with dynamic children', () => {
    ...
    test('CREATE: an element generated by a <template if> inside <svg> stays SVG', () => {
        const host = document.createElement('div')
        runBuild(SVG_MIXED_STATIC, host, false)
