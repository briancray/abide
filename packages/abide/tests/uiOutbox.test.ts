...
const queueFor = (url: string) =>
    createOutboxQueue<{ n: number }>({
        url,
        online: () => false, // stay offline so nothing drains
        store: memoryStore(),
...
describe('global outbox() aggregate', () => {
    test('flattens entries across durable rpcs, each tagged with its rpc', () => {
        ...
        queueA.enqueue({ n: 1 }, new Request('http://localhost/rpc/aggA', { method: 'POST' }))
        queueB.enqueue({ n: 2 }, new Request('http://localhost/rpc/aggB', { method: 'POST' }))
