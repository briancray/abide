...
only on `ssr.awaits.length > 0` — and drains it into an inline `__abideResolve(...)` chunk. A
pending read is bounded by its OWN endpoint `timeout` (which 504s the in-process handler during
SSR) — there is NO separate SSR-stream deadline (option a). A page with NO async work stays
buffered exactly as before; a non-replayable read (producer / stream cell — no wire request)
never triggers streaming.
...
/* A GET whose handler never settles but declares a short `timeout` — the endpoint timeout 504s it
   in-process during SSR, so the drain ships a `{ key, miss }` and closes (no separate deadline). */
const getSlow = defineRpc('GET', '/rpc/bare-slow', () => new Promise<Response>(() => {}), {
    timeout: 30,
})
...
describe('ADR-0024 bare-read auto-streaming', () => {
    ...
    /* Done-criteria #2 (option a — the endpoint timeout bounds the stream, no separate SSR deadline):
       a bare read whose handler hangs but declares a `timeout` is 504'd in-process during SSR, so the
       entry settles, the drain ships a `{ key, miss }`, and the stream CLOSES. The test completing at
       all proves the endpoint timeout alone prevents a hang. The 2s per-test cap fails fast (rather
       than hanging) if the in-process timeout ever regresses. */
    test('a hung read is bounded by its endpoint timeout — ships a miss and closes, no SSR deadline', async () => {
        const html = await render(
            page(() => {
                /* Trigger the read and swallow its expected 504 rejection — the component reads via a
                   non-awaiting peek in real use; the drain independently handles the entry (readSettled). */
                cache(getSlow).catch(() => {})
                return { html: '<main>shell</main>', awaits: [], state: undefined, resume: {} }
            }),
        )
        ...
        expect(html).toContain('GET /rpc/bare-slow') // the timed-out entry key
        expect(html).toContain('"miss":true') // 504'd at its timeout → miss → client refetches
        expect(html).toContain('</html>') // stream reached its tail and closed — no hang
    }, 2000)
