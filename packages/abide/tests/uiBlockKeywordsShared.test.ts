...
(`parseTemplate`) and the LSP highlighter (`structuralBlockTokens`) — the split that
once left `{#snippet}` heads uncolored. Every opener the shared list names must (1) be a
real block the parser accepts and (2) get a `keyword` token from the highlighter. A new
control block added to the parser but not the list breaks its own dispatch first; one
listed but not colored fails here.
*/
...
describe('BLOCK_KEYWORDS shared vocabulary', () => {
    for (const opener of BLOCK_OPENERS) {
        ...
        test(`the highlighter colors {#${opener}} as a keyword`, () => {
            const tokens = structuralBlockTokens(`{#${opener} x}`)
    ...
    test('regression: {#snippet} is colored (the head the hand-copy once dropped)', () => {
        const tokens = structuralBlockTokens('{#snippet row(x)}')
