...
describe('keyed each', () => {
    ...
    test('sync generator items: materialized fresh each run, stays reactive', () => {
        const model = doc({ items: [{ id: 'a' }, { id: 'b' }] })
        const root = host()
        const list = document.createElement('ul')
        mount(root, (h) => {
            each(
                list,
                /* A generator (fresh each read) — exercises the non-array iterable path;
                   `read` runs while materializing, so the block stays reactive. */
                function* () {
                    for (const item of model.read<{ id: string }[]>('items')) {
                        yield item
                    }
                },
                (item) => item.id,
        ...
        expect([...list.children].map((c) => c.getAttribute('data-id'))).toEqual(['a', 'b'])
        model.replace('items', [{ id: 'b' }, { id: 'a' }, { id: 'c' }])
        expect([...list.children].map((c) => c.getAttribute('data-id'))).toEqual(['b', 'a', 'c'])
    })
})
