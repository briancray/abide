...
- 3941314: address every component mount as its own hydration boundary so a structural desync inside a child recovers at that one boundary instead of discarding the whole page (ADR-0049)
...
- e137242: `spreadAttrs` — swallow a per-key suspend on a `{...rest}` spread
...
- 7367a23: SSR hydration payload ships as inert JSON instead of executable JavaScript (ADR-0051)
