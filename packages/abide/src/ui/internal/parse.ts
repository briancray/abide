...
export function parse(source: string, opts?: { filename?: string }): Root {
    ...
    function readTagName(): string {
        ...
        while (pos < length) {
            const char = source[pos]
            if (char === undefined || !/[A-Za-z0-9._-]/.test(char)) break
            pos++
        }
    ...
    function readIdentifier(): string {
        ...
        while (pos < length) {
            const char = source[pos]
            if (char === undefined || !IDENT_CHAR.test(char)) break
            pos++
        }
        return source.slice(start, pos)
    }
    ...
    function parseRawText(start: number, name: string, isScript: boolean): Script | Style {
        ...
        if (isScript) {
            const module = attributes.some(
                (attr) => attr.type === 'StaticAttribute' && attr.name === 'module',
            )
            const node: Script = {
                type: 'Script',
                module,
    ...
    function parseAttribute(): AttributeNode {
        ...
        if (name.startsWith('bind:')) {
            return {
                type: 'BindDirective',
                name: name.slice(5),
                expression: directiveExpression(name, valueKind, expression),
                start,
                end,
            }
        }
    ...
    function parseAwaitBlock(start: number): AwaitBlock {
        ...
        while (atClauseOpen()) {
            ...
            if (keyword === 'then') {
                if (thenClause !== null)
                    fail(
                        '`{#await}` already has a `then` branch (from the inline `then` in the opener)',
                        clauseStart,
                    )
                const param = readBraceContents().trim() || null
                ...
                if (catchClause !== null)
                    fail(
                        '`{#await}` already has a `catch` branch (from the inline `catch` in the opener)',
                        clauseStart,
                    )
                const param = readBraceContents().trim() || null
        ...
        return {
            ...
            expression,
            pending,
            // biome-ignore lint/suspicious/noThenProperty: AwaitBlock AST node field for the `{:then}` clause, not a thenable
            then: thenClause,
            catch: catchClause,
            finally: finallyClause,
            inline: inlineThen !== null || inlineCatch !== null,
            start,
            end: pos,
        }
    }
