...
async function runStep(command: string[], cwd: string): Promise<boolean> {
  try {
    ...
    console.error(`abide scaffold: \`${command.join(" ")}\` failed:`, caught instanceof Error ? caught.message : String(caught));
    return false;
  }
}
...
export async function build(dir: string): Promise<string> {
  ...
  await Bun.write(join(outDir, "index.json"), JSON.stringify({ hash, entry: "client.js" }, null, 2));
  return outDir;
}
...
export async function scaffold(dir: string, name: string): Promise<string> {
  ...
  for (const relative of Object.keys(files)) {
    await Bun.write(join(root, relative), files[relative]!);
  }
  return root;
}
...
async function forwardLsp(cwd: string): Promise<void> {
  ...
  const child = Bun.spawn(["node", lspPath], { cwd, stdin: "pipe", stdout: "pipe", stderr: "inherit" });
  const pumpIn = (async () => {
...
export async function main(argv: string[]): Promise<ServeResult | undefined> {
  ...
  if (command === "lsp") {
    ...
    if (bunCanHostTsgo()) {
      await lspServer({ projectRoot: cwd, read: Bun.stdin.stream(), write: (bytes) => void process.stdout.write(bytes) });
    } else {
  ...
  if (command === "bundle") {
    ...
    console.info(`  note: native windowing is best-effort (system webview binary or default browser)`);
    return undefined;
  }
