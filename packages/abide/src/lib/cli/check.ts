...
import { emitCheck, componentDts, mapGenToOrig, CHECK_HEADER_LENGTH, type Segment } from "../ui/internal/emitCheck.ts";
...
export async function check(dir: string): Promise<CheckResult> {
  ...
  const rawDiagnostics =
    toCheck.length > 0 ? diagnose(dir, { files: virtualFiles, open: toCheck.map((entry) => entry.tempPath) }) : [];
  ...
  diagnostics.sort((a, b) => (a.file === b.file ? a.line - b.line || a.column - b.column : a.file < b.file ? -1 : 1));
...
function buildGenerated(abidePath: string, source: string, root: Root): Generated {
  ...
  const tempPath = join(directory, `__abide_check_${basename(abidePath).replace(/[^\w]/g, "_")}_${Bun.hash(abidePath).toString(36)}.ts`);
...
export function overlayFs(getFiles: () => Record<string, string>): FileSystem {
  return {
    ...
    fileExists: (name) => (Object.prototype.hasOwnProperty.call(getFiles(), name) ? true : undefined),
    realpath: (path) => (Object.prototype.hasOwnProperty.call(getFiles(), path) ? path : undefined),
...
function diagnoseViaNodeSubprocess(cwd: string, request: DiagnoseRequest): RawDiagnostic[] {
  ...
  const proc = spawnSync(["node", self, "--abide-diagnose", cwd], { stdin: new TextEncoder().encode(JSON.stringify(request)), stdout: "pipe", stderr: "pipe" });
  const stdout = proc.stdout.toString();
  const markerAt = stdout.lastIndexOf(RESULT_MARKER);
  if (markerAt === -1) {
    const stderr = proc.stderr.toString().trim();
    throw new Error(`abide check: type-check subprocess produced no result${stderr ? ` (stderr: ${stderr})` : ""}`);
  }
...
export function diagnoseInProcess(cwd: string, request: DiagnoseRequest): RawDiagnostic[] {
  ...
  try {
    ...
    for (const file of request.open) {
      ...
      const collected = [...program.getSyntacticDiagnostics(file), ...program.getSemanticDiagnostics(file)];
