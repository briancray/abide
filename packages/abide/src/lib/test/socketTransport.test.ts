...
describe("socket transport — WebSocket mux", () => {
  test("a WS subscriber receives a server-side publish", async () => {
  ...
  test("tail replay — a late subscriber replays the last N messages", async () => {
    const ticks = socket<number>({ clientPublish: true, tail: 2 });
    const app = start({ sockets: { ticks } });
  ...
  test("client publish over the WS reaches subscribers", async () => {
  ...
  test("client publish is ignored when clientPublish is off", async () => {
...
describe("socket transport — HTTP face", () => {
  test("POST publishes a client message that reaches WS subscribers", async () => {
    const ticks = socket<string>({ clientPublish: true, tail: 2 });
    const app = start({ sockets: { ticks } });
  ...
  test("POST is rejected 403 when clientPublish is off", async () => {
    const quiet = socket<string>({ clientPublish: false });
    const app = start({ sockets: { quiet } });
  ...
  test("GET streams messages over SSE", async () => {
    const ticks = socket<string>({ clientPublish: true, tail: 2 });
    const app = start({ sockets: { ticks } });
  ...
  test("unknown socket 404s on the HTTP face", async () => {
...
describe("socket transport — CSWSH", () => {
  test("a WS upgrade with a foreign Origin is rejected when APP_URL is set", async () => {
    ...
    try {
      ...
      const response = await app.fetch("/__abide/sockets", {
        headers: { origin: "http://evil.example", upgrade: "websocket", connection: "Upgrade" },
      });
      expect(response.status).toBe(403);
    } finally {
