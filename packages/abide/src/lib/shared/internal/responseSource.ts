...
export type ResponseSource =
  ...
  | { kind: "stream"; source: AsyncIterable<unknown> | Iterable<unknown>; encoding: "jsonl" | "sse" };
...
export type Payload<R> = R extends StreamResponse<infer C>
  ? AsyncIterable<C>
...
export function tagResponseSource(response: Response, source: ResponseSource): Response {
  Object.defineProperty(response, RESPONSE_SOURCE, { value: source, enumerable: false, configurable: true });
  return response;
}
...
export function tagStreamEncoding(cursor: AsyncIterable<unknown>, encoding: "jsonl" | "sse"): void {
  Object.defineProperty(cursor, STREAM_ENCODING, { value: encoding, enumerable: false, configurable: true });
}
