...
import { shapeToSchema, jsonSchemaOf } from "./shapeToSchema.ts";
...
describe("shapeToSchema", () => {
  test("drops undeclared object fields (the passwordHash leak)", () => {
    const schema: JSONSchema = { type: "object", properties: { id: { type: "number" }, name: { type: "string" } } };
  ...
  test("keeps undeclared fields when additionalProperties is true", () => {
    const schema: JSONSchema = { type: "object", properties: { id: { type: "number" } }, additionalProperties: true };
  ...
  test("shapes array items", () => {
    const schema: JSONSchema = { type: "array", items: { type: "object", properties: { id: { type: "number" } } } };
    const shaped = shapeToSchema([{ id: 1, secret: "a" }, { id: 2, secret: "b" }], schema);
...
describe("jsonSchemaOf", () => {
  ...
  test("returns undefined for a Standard Schema (opaque field set)", () => {
    const standard: StandardSchemaV1 = { "~standard": { version: 1, vendor: "test", validate: (v) => ({ value: v }) } };
