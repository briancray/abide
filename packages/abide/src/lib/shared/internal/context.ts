...
export interface StreamScope {
  ...
  // Resolves (to a sentinel) after the total `{#for await}` streaming budget (default 30s). A streamed
  // list whose source ENDS before it is marked complete (client claims the items); one still going when
  // it fires is cut off (client re-iterates) — this is why an SSR `{#for await}` never hangs the body.
  streamBudget: Promise<symbol>;
...
const requestStorage: AsyncLocalStorage<CacheContext> | undefined = isBrowser ? undefined : new AsyncLocalStorage<CacheContext>();
