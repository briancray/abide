...
import { type JSONSchema, toStandard, validateJsonSchema } from './jsonSchema.ts'
...
describe('type keyword', () => {
    ...
    test('mismatched type reports an issue', () => {
        ...
        if (!result.ok) {
            ...
            expect(issue.message).toContain('Expected type string')
...
describe('combinators', () => {
    ...
    test('anyOf rejects when no branch matches', () => {
        ...
        if (!result.ok) {
            const issue = result.issues[0]
            if (!issue) throw new Error('expected at least one issue')
            expect(issue.message).toContain('any of')
        }
    })
...
describe('string constraints + format', () => {
    test('minLength / maxLength', () => {
        ...
        expect(validateJsonSchema({ type: 'string', minLength: 1, maxLength: 3 }, 'ab').ok).toBe(
            true,
        )
    })
    ...
    test('date-time format accepts ISO strings', () => {
        expect(
            validateJsonSchema({ type: 'string', format: 'date-time' }, '2026-07-17T12:00:00Z').ok,
        ).toBe(true)
        expect(
            validateJsonSchema(
                { type: 'string', format: 'date-time' },
                '2026-07-17T12:00:00.123+02:00',
            ).ok,
        ).toBe(true)
    })
    ...
    test('date-time format rejects non-ISO strings', () => {
        ...
        if (!result.ok) {
            const issue = result.issues[0]
            if (!issue) throw new Error('expected at least one issue')
            expect(issue.message).toContain('ISO')
        }
    })
})
