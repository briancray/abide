...
function validateNode(schema: JSONSchema, value: unknown, path: Array<string | number>, issues: Issue[]): void {
  ...
  if (schema.enum !== undefined) {
    ...
    if (!matched) {
      issues.push({ message: `Expected one of ${schema.enum.map(stringify).join(", ")}`, path });
    }
  }
...
function validateObject(schema: JSONSchema, value: Record<string, unknown>, path: Array<string | number>, issues: Issue[]): void {
  ...
  if (schema.properties !== undefined) {
    for (const key in schema.properties) {
      if (key in value) {
        validateNode(schema.properties[key]!, value[key], [...path, key], issues);
      }
    }
  }
...
function validateString(schema: JSONSchema, value: string, path: Array<string | number>, issues: Issue[]): void {
...
function validateCombinators(schema: JSONSchema, value: unknown, path: Array<string | number>, issues: Issue[]): void {
  ...
  if (schema.oneOf !== undefined) {
    ...
    if (matches !== 1) issues.push({ message: `Expected to match exactly one schema, matched ${matches}`, path });
  }
