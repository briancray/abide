...
refresh / peek, `fn.patch(args?, updater)` ≡ `patch(fn, args, updater)`,
...
(defineRpc) and client (remoteProxy) shapes are identical. `patch` is attached uniformly;
the type omits it for a streaming rpc (harmless at runtime — a stream has no cache entry).
*/
