...
import { loadEmitted, type ComponentResolver } from "./emit.ts";
...
const resolve: ComponentResolver = (specifier) => (specifier === "./Component.abide" ? COMPONENT : undefined);
...
describe("file-component — SSR composition", () => {
  ...
  test("an unresolved component import fails loudly", async () => {
    ...
    await expect(loadEmitted(PAGE, () => undefined)).rejects.toThrow(/could not resolve component import/);
  });
});
