  ...
  test("every recorded segment is a byte-identical copy of the .abide source", () => {
    for (const source of CORPUS) {
      ...
      for (const segment of segments) {
        ...
        const orig = source.slice(segment.origStart, segment.origStart + (segment.genEnd - segment.genStart));
  ...
  test("segments are monotonic in both gen and orig offsets", () => {
    for (const source of CORPUS) {
      ...
      for (let i = 1; i < segments.length; i++) {
        expect(segments[i]!.genStart).toBeGreaterThanOrEqual(segments[i - 1]!.genEnd);
      }
    }
  });
});
