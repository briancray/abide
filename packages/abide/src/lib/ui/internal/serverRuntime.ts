...
export function applyStyleDir(builder: AttributeBuilder, name: string, value: unknown): void {
  if (value !== false && value !== null && value !== undefined) builder.addStyle(`${name}: ${String(value)}`);
}
...
function resolveBound(bound: unknown): unknown {
  ...
  if (bound !== null && typeof bound === "object") {
    ...
    if (typeof object.get === "function" && typeof object.set !== "undefined") return object.get();
  }
  return bound;
}
...
export function applyBind(builder: AttributeBuilder, name: string, value: unknown): void {
  ...
  if (name === "group") {
    ...
    const checked = isCheckbox ? Array.isArray(current) && current.includes(inputValue) : current === inputValue;
