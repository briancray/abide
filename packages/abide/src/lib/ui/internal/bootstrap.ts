...
import { registerPages, type PageEntry, type PageMount, type RpcSpecs } from "./pageRegistry.ts";
...
function replayReads(seed: HydrationSeed, imports: Record<string, unknown>): void {
  ...
  for (const record of reads) {
    ...
    const proxy = imports[record.name] as { seed?: (args: unknown, value: unknown) => void } | undefined;
...
export function bootstrapPage(
  ...
  const scope: Record<string, unknown> = { ...imports, state: makeSeededState(seed), watch, props: () => props };
...
export function bootstrapApp(pages: Record<string, PageEntry>, rpcSpecs: RpcSpecs, base?: string): () => void {
  if (typeof document === "undefined") return () => {};
