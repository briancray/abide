...
import type { ScopeAnalysis, ScriptInfo, ImportBinding } from "./analyzeScope.ts";
...
function importLocals(imports: ImportBinding[], skip?: Set<string>): string[] {
  ...
  for (const binding of imports) {
    if (binding.defaultLocal !== null && !skip?.has(binding.defaultLocal)) locals.push(binding.defaultLocal);
    if (binding.namespaceLocal !== null && !skip?.has(binding.namespaceLocal)) locals.push(binding.namespaceLocal);
...
function moduleBindingNames(script: ScriptInfo, componentLocals: Set<string>): string[] {
  ...
  for (const local of importLocals(script.imports, componentLocals)) {
    if (!seen.has(local)) { seen.add(local); names.push(local); }
  }
...
function instanceDeclaredNames(script: ScriptInfo | null, componentLocals: Set<string>): Set<string> {
  const names = new Set<string>();
  if (script === null) return names;
  for (const local of importLocals(script.imports, componentLocals)) names.add(local);
  for (const binding of script.bindings) if (!componentLocals.has(binding.name)) names.add(binding.name);
  return names;
}
...
export function emitInstanceSetup(analysis: ScopeAnalysis): string {
  ...
  if (analysis.module !== null) {
    ...
    const moduleImports = importLocals(analysis.module.imports, componentLocals).filter((name) => !shadowed.has(name));
    ...
    if (declNames.length > 0) out += `  const { ${declNames.join(", ")} } = $ensureModule($scope);\n`;
    else out += `  $ensureModule($scope);\n`;
    for (const name of moduleImports) out += `  const ${name} = $scope[${JSON.stringify(name)}];\n`;
  }
