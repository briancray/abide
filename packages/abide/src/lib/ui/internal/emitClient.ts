...
import type { ClientPlan, DynamicSlot, TemplatePlan, AttrPlan } from "./templatePlan.ts";
...
class ClientEmitter {
  ...
  emit(plan: TemplatePlan): string {
    const rootPlan: ClientPlan = { skeleton: plan.skeletonClient, slots: plan.slots, elementTags: plan.elementTags };
    this.idFor(rootPlan); // id 0
    // Generating mount fns grows `this.plans` as sub-plans are discovered.
    let fns = "";
    for (let i = 0; i < this.plans.length; i++) {
      fns += this.genMountFn(this.plans[i]!.id, this.plans[i]!.plan);
    }
    let templates = "";
  ...
  private genMountFn(id: number, plan: ClientPlan): string {
    ...
    const parentOf = (path: number[]): string => (path.length <= 1 ? "$target" : nav(path.slice(0, -1)));
    ...
    const decls = allPaths.length > 0 ? `  let ${allPaths.map((p) => `$n${p.join("_")}`).join(", ")};\n` : "";
  ...
  private genClonePositional(paths: number[][]): string {
    const sorted = paths.slice().sort((a, b) => a.length - b.length || a.join(",").localeCompare(b.join(",")));
    let code = "";
  ...
  private genHydrateLevel(slots: DynamicSlot[], prefix: number[], tags: Map<string, string>): string {
    ...
    for (const [index, bucket] of groups) {
      ...
      else if (leafSlot !== undefined) entries.push({ start: index, index, kind: "leaf", leafKind: leafSlot.kind });
    ...
    for (const entry of entries) {
      ...
      if (entry.kind === "leaf") {
        ...
        if (tag !== undefined) code += `${varName} = $rt.claimElement($rt.hydrateNode(), ${JSON.stringify(tag)});\n`;
  ...
  private openRef(slot: DynamicSlot, nav: (p: number[]) => string): string {
    ...
    const openPath = [...path.slice(0, -1), path[path.length - 1]! - 1];
    return nav(openPath);
  }
  ...
  private genSnippet(slot: DynamicSlot): string {
    ...
    let binds = "";
    for (let i = 0; i < patterns.length; i++) binds += "    " + bindPattern("$s", patterns[i]!, `$args[${i}]`) + "\n";
    const bodyId = this.idFor(slot.meta.body!);
    return (
  ...
  private genSlot(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
  ...
  private genIf(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    const branches = slot.meta.branches!
      .map((b) => {
  ...
  private genSwitch(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    const cases = slot.meta.branches!
      .map((c) => {
        const test = c.expr === null ? "null" : `() => (${c.expr})`;
        return `{ test: ${test}, body: ${this.blockFn(c.plan, "$scope")} }`;
      })
      .join(", ");
    const leading = this.blockFn(slot.meta.leading!, "$scope");
  ...
  private genFor(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    ...
    if (slot.meta.catch) {
      ...
      const child = c.param ? `(() => { const $c = Object.create($scope); ${bindPattern("$c", c.param, "$error")} return $c; })()` : "$scope";
  ...
  private genAwaitBlock(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    const pending = this.blockFn(slot.meta.pending!, "$scope");
  ...
  private genTry(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    const body = this.blockFn(slot.meta.body!, "$scope");
    const catchFn = slot.meta.catch
      ? `($error) => ${this.paramBlockFn(slot.meta.catch.plan, slot.meta.catch.param, "$error")}`
      : "null";
    const finallyFn = slot.meta.finally ? this.blockFn(slot.meta.finally, "$scope") : "null";
  ...
  private genComponent(slot: DynamicSlot, nav: (p: number[]) => string, parentOf: (p: number[]) => string): string {
    const name = slot.meta.name!;
    ...
    const childrenFn = slot.meta.hasChildren ? `() => (${this.mountable(slot.meta.body!, "$scope")})` : "null";
