...
function parseAttachSource(iterable: string, rpcLocals: Set<string>): { rpcName: string; args: string } | null {
...
function bindPattern(target: string, pattern: string, valueExpr: string): string {
  ...
  if (/^[A-Za-z_$][\w$]*$/.test(trimmed)) return `${target}[${JSON.stringify(trimmed)}] = ${valueExpr};`;
...
function genComponent(
  ...
  if (hasChildren) out += `    const $children = async () => new $rt.Raw(await ${bodyExpr(analysis, children)}($scope));\n`;
...
function genChunkRaw(analysis: ScopeAnalysis, chunk: ServerChunk): string {
  switch (chunk.kind) {
    ...
    case "element":
      return genElement(analysis, chunk.name, chunk.void, chunk.attrs, chunk.children, chunk.scopeAttr);
    case "component":
      return genComponent(analysis, chunk.name, chunk.attrs, chunk.children, chunk.hasChildren);
    case "if": {
      let out = "  {\n    let $r = \"\";\n";
      let first = true;
      let hasElse = false;
    ...
    case "awaitBlock": {
      ...
      if (!chunk.inline) {
        ...
        const caught = chunk.catch
          ...
          : "null";
        const finalize = chunk.finally ? `async () => await ${bodyExpr(analysis, chunk.finally)}($scope)` : "null";
      ...
      if (chunk.then) {
        out += "      " + childScopeCode("$ct", chunk.then.param, "$value").replace(/\n/g, "\n      ");
      ...
      out += "    } catch ($e) {\n";
      if (chunk.catch) {
        out += "      " + childScopeCode("$cc", chunk.catch.param, "$e").replace(/\n/g, "\n      ");
        out += `      $out += await ${bodyExpr(analysis, chunk.catch.children)}($cc);\n`;
      } else {
        out += "      throw $e;\n";
      }
      out += "    }\n";
      if (chunk.finally) out += `    $out += await ${bodyExpr(analysis, chunk.finally)}($scope);\n`;
      out += "    return $out;\n  })($scope);\n";
      return out;
    }
    case "switch": {
      ...
      if (fallback) out += `    return await ${bodyExpr(analysis, fallback.children)}($scope);\n`;
      out += "    return \"\";\n  })($scope);\n";
      return out;
    }
    case "try": {
