...
function analyzeBraces(tokens: Tok[]): BraceInfo {
  ...
  for (let i = 0; i < n; i++) {
    ...
    enclBraceOpen[i] = braceStack.length ? braceStack[braceStack.length - 1]! : -1;
  ...
  for (let i = 0; i < n; i++) {
    if (tokens[i]!.kind === K.OpenBraceToken) {
      const prev = i > 0 ? tokens[i - 1]!.kind : undefined;
...
function collectParamBindings(tokens: Tok[], open: number, close: number, filter: (name: string) => boolean): number[] {
  const params: number[] = [];
  let depth = 0;
  let expectName = true;
...
function collectVarBindings(tokens: Tok[], kw: number, filter: (name: string) => boolean): number[] {
  const names: number[] = [];
  let depth = 0;
...
function arrowExprEnd(tokens: Tok[], start: number): number {
  ...
  for (let j = start; j < tokens.length; j++) {
    ...
    if (depth === 0 && (kind === K.CommaToken || kind === K.SemicolonToken)) return j > start ? j - 1 : start;
    if (isOpen(kind)) depth++;
    else if (isClose(kind)) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (kind === K.LetKeyword || kind === K.ConstKeyword || kind === K.VarKeyword) {
      ...
      for (const idx of idxs) {
        ...
        if (!isTopLevel) shadows.push({ name: tokens[idx]!.text, start: i, end: enclClose(i) });
      }
      continue;
    }
    ...
    if (kind === K.FunctionKeyword) {
      let j = i + 1;
      if (tokens[j] && tokens[j]!.kind === K.AsteriskToken) j++; // generator `*`
      let nameIdx = -1;
      if (tokens[j] && isIdentifierLike(tokens[j]!.kind)) {
        nameIdx = j;
        if (filter(tokens[j]!.text)) {
          declNameIdx.add(j);
          if (bracketDepth[i]! > 0) shadows.push({ name: tokens[j]!.text, start: i, end: enclClose(i) });
        }
      }
      let p = nameIdx !== -1 ? nameIdx + 1 : j;
      while (p < n && tokens[p]!.kind !== K.OpenParenToken) p++;
      if (p < n && tokens[p]!.kind === K.OpenParenToken) {
        const close = matchClose.get(p);
        if (close !== undefined) {
          const params = collectParamBindings(tokens, p, close, filter);
          let b = close + 1;
          while (b < n && tokens[b]!.kind !== K.OpenBraceToken) b++;
          if (b < n && tokens[b]!.kind === K.OpenBraceToken) {
            const bclose = matchClose.get(b) ?? n - 1;
            for (const pidx of params) {
              declNameIdx.add(pidx);
              shadows.push({ name: tokens[pidx]!.text, start: b, end: bclose });
            }
          } else {
    ...
    if (kind === K.ClassKeyword) {
      const j = i + 1;
      if (tokens[j] && isIdentifierLike(tokens[j]!.kind) && filter(tokens[j]!.text)) {
        declNameIdx.add(j);
        if (bracketDepth[i]! > 0) shadows.push({ name: tokens[j]!.text, start: i, end: enclClose(i) });
      }
      continue;
    ...
    if (kind === K.EqualsGreaterThanToken) {
      ...
      if (prev) {
        if (prev.kind === K.CloseParenToken) {
          ...
          if (open !== undefined) params = collectParamBindings(tokens, open, i - 1, filter);
      ...
      const bodyStart = i + 1;
      let bodyEnd: number;
      if (tokens[bodyStart] && tokens[bodyStart]!.kind === K.OpenBraceToken) {
      ...
      for (const pidx of params) {
        declNameIdx.add(pidx);
        shadows.push({ name: tokens[pidx]!.text, start: bodyStart, end: bodyEnd });
      }
    }
  }
...
export function rewriteCellRefs(code: string, cellNames: Set<string>): string {
    ...
    for (;;) {
      ...
      for (let k = 0; k < pending.length; k++) {
        ...
        if (best === -1) best = k;
          ...
          if (c.pos < b.pos || (c.pos === b.pos && c.seq > b.seq)) best = k;
        }
      }
      if (best === -1) break;
      const c = pending[best]!;
      if (c.pos > cursor) {
        out += code.slice(cursor, c.pos);
        cursor = c.pos;
      }
      out += c.text;
  ...
  while (i < tokens.length) {
    ...
    if (kind === K.PlusPlusToken || kind === K.MinusMinusToken) {
      ...
      if (isPrefix && next && next.kind === K.Identifier && cellNames.has(next.text) && isCellRef(i + 1)) {
        flush(t.start);
    ...
    if (kind === K.Identifier && cellNames.has(t.text) && isCellRef(i)) {
      ...
      if (nextKind === K.EqualsToken) {
        // `n = rhs` → `n.write(rhs)`
        flush(t.start);
        out += `${name}.write(`;
        const end = rhsExtent(tokens, i + 2);
        pending.push({ pos: tokens[end]!.end, text: ")", seq: seq++ });
        cursor = next!.end; // skip `n` and `=`
        i += 2;
        continue;
      }

      if (nextKind !== undefined && COMPOUND_OP.has(nextKind)) {
        // `n op= rhs` → `n.write(n.read() op (rhs))`
        flush(t.start);
        const op = COMPOUND_OP.get(nextKind)!;
      ...
      if (nextKind === K.GreaterThanToken) {
        ...
        let run = i + 1;
        while (tokens[run] && tokens[run]!.kind === K.GreaterThanToken) run++;
        const count = run - (i + 1);
        if ((count === 2 || count === 3) && tokens[run] && tokens[run]!.kind === K.EqualsToken) {
          flush(t.start);
          const op = count === 2 ? ">>" : ">>>";
          out += `${name}.write(${name}.read() ${op} (`;
          const end = rhsExtent(tokens, run + 1);
          pending.push({ pos: tokens[end]!.end, text: "))", seq: seq++ });
          cursor = tokens[run]!.end; // skip `n`, the `>` run, and `=`
          i = run + 1;
          continue;
        }
      }

      if (nextKind === K.PlusPlusToken || nextKind === K.MinusMinusToken) {
        // postfix `n++` / `n--`
        flush(t.start);
        const op = nextKind === K.PlusPlusToken ? "+" : "-";
        out += `${name}.write(${name}.read() ${op} 1)`;
        cursor = next!.end;
        i += 2;
        continue;
...
function markInner(tokens: Tok[], open: number, close: number, skip: Set<number>): void {
  for (let j = open + 1; j < close; j++) {
    if (isIdentifierLike(tokens[j]!.kind)) skip.add(j);
  }
}
...
function scanAngle(tokens: Tok[], openIdx: number, matchClose: Map<number, number>, skip: Set<number>): number {
  let depth = 0;
...
function scanTypeAtom(tokens: Tok[], start: number, matchClose: Map<number, number>, skip: Set<number>): number {
  let i = start;
  ...
  while (i < tokens.length && LEADING_TYPE_OPS.has(tokens[i]!.kind)) {
    if (isIdentifierLike(tokens[i]!.kind)) skip.add(i);
    i++;
  }
  const k = tokens[i]?.kind;
  ...
  if (isIdentifierLike(k)) {
    ...
    while (tokens[i]?.kind === K.DotToken && tokens[i + 1] && isIdentifierLike(tokens[i + 1]!.kind)) {
      skip.add(i + 1);
      i += 2;
    }
...
function scanTypeOperand(tokens: Tok[], start: number, matchClose: Map<number, number>, skip: Set<number>): void {
  let i = start;
  for (;;) {
...
function markTypeSkips(tokens: Tok[], matchClose: Map<number, number>): Set<number> {
  ...
  for (let i = 0; i < tokens.length; i++) {
    const k = tokens[i]!.kind;
    if (k === K.AsKeyword || k === K.SatisfiesKeyword) scanTypeOperand(tokens, i + 1, matchClose, skip);
  }
  return skip;
}
...
export function rewriteFreeIdentifiers(code: string, declared: Set<string>, scopeVar: string): string {
  const tokens = tokenize(code);
  if (tokens.length === 0) return code;
  const braces = analyzeBraces(tokens);
  const { enclBraceOpen, isObjectBrace } = braces;
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const encl = enclBraceOpen[i]!;
    const inObjectPosition = encl !== -1 && isObjectBrace.has(encl) && (prev === K.OpenBraceToken || prev === K.CommaToken);
    if (inObjectPosition) {
      const next = tokens[i + 1]?.kind;
...
export function collectFreeIdentifiers(expr: string, declared: Set<string>): Set<string> {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const encl = enclBraceOpen[i]!;
    if (encl !== -1 && isObjectBrace.has(encl) && (prev === K.OpenBraceToken || prev === K.CommaToken)) {
      const next = tokens[i + 1]?.kind;
...
function parseImport(rawText: string): ImportBinding | null {
  ...
  const clause = rawText.slice(rawText.indexOf("import") + "import".length, specifierMatch.index).trim();
  const binding: ImportBinding = { specifier, defaultLocal: null, namespaceLocal: null, named: [] };

  const namespaceMatch = clause.match(/\*\s*as\s+([A-Za-z_$][\w$]*)/);
  if (namespaceMatch) binding.namespaceLocal = namespaceMatch[1]!;

  const bracedMatch = clause.match(/\{([^}]*)\}/);
  if (bracedMatch) {
    for (const entry of bracedMatch[1]!.split(",")) {
      const trimmed = entry.trim();
      if (trimmed === "") continue;
      const asMatch = trimmed.match(/^([A-Za-z_$][\w$]*)\s+as\s+([A-Za-z_$][\w$]*)$/);
      if (asMatch) binding.named.push({ imported: asMatch[1]!, local: asMatch[2]! });
      else binding.named.push({ imported: trimmed, local: trimmed });
    }
  }

  const beforeBrace = clause.replace(/\{[^}]*\}/, "").replace(/\*\s*as\s+[A-Za-z_$][\w$]*/, "");
  const defaultMatch = beforeBrace.match(/^\s*([A-Za-z_$][\w$]*)\s*,?/);
  if (defaultMatch && defaultMatch[1] !== "") binding.defaultLocal = defaultMatch[1]!;
...
type StatementRecord =
  | { kind: "import"; binding: ImportBinding | null; cssSpecifier: string | null; stripStart: number; stripEnd: number }
...
function cssSideEffectSpecifier(rawText: string): string | null {
  const match = rawText.match(/^\s*import\s*['"]([^'"]+)['"]/);
  ...
  while (i < n) {
    ...
    if (depth === 0 && atStart) {
      ...
      if (kind === K.ImportKeyword) {
        ...
        const rawText = source.slice(t.start, tokens[lastIdx]!.end);
        const terminator = nextIdx > 0 && nextIdx <= n && tokens[nextIdx - 1] && tokens[nextIdx - 1]!.kind === K.SemicolonToken;
        const stripEnd = terminator ? tokens[nextIdx - 1]!.end : tokens[lastIdx]!.end;
        const binding = parseImport(rawText);
        const cssSpecifier = binding === null ? cssSideEffectSpecifier(rawText) : null;
        records.push({ kind: "import", binding, cssSpecifier, stripStart: t.start, stripEnd });
        i = nextIdx;
        atStart = true;
        continue;
      }
      if (kind === K.LetKeyword || kind === K.ConstKeyword || kind === K.VarKeyword) {
        const { lastIdx, nextIdx } = stmtSpan(i);
        const rawDeclarators = source.slice(t.end, tokens[lastIdx]!.end);
      ...
      if (kind === K.FunctionKeyword || kind === K.ClassKeyword) {
        let j = i + 1;
        if (tokens[j] && tokens[j]!.kind === K.AsteriskToken) j++;
        const name = tokens[j] && isIdentifierLike(tokens[j]!.kind) ? tokens[j]!.text : null;
...
function localForSpecifier(imports: ImportBinding[], specifier: string, importedName: string, fallback: string): string {
...
function analyzeScript(content: string): RawScript {
  ...
  for (const record of records) {
    if (record.kind === "import") {
      ...
      if (record.binding) {
        ...
        if (record.binding.specifier.endsWith(".abide") && record.binding.defaultLocal !== null) {
          componentImports.push({ local: record.binding.defaultLocal, specifier: record.binding.specifier });
  ...
  return { imports, bindings, cells, declared, cssImports, componentImports, moduleImports, strippedCode };
}
...
export function analyzeScope(root: Root): ScopeAnalysis {
  ...
  const module: ScriptInfo | null = moduleRaw
    ? { setupCode: rewriteCellRefs(moduleRaw.strippedCode, moduleRaw.cells), imports: moduleRaw.imports, bindings: moduleRaw.bindings, cssImports: moduleRaw.cssImports }
    : null;
  const instance: ScriptInfo | null = instanceRaw
    ? { setupCode: rewriteCellRefs(instanceRaw.strippedCode, cellNames), imports: instanceRaw.imports, bindings: instanceRaw.bindings, cssImports: instanceRaw.cssImports }
    : null;
