...
function only<T extends TemplateNode>(source: string): T {
  const significant = children(source).filter((node) => !(node.type === "Text" && node.value.trim() === ""));
...
describe("attributes and directives", () => {
  ...
  test("multiple attributes", () => {
    ...
    expect(list.map((a) => a.type)).toEqual(["StaticAttribute", "EventAttribute", "ClassDirective", "SpreadAttribute"]);
  });
  ...
  test("attribute expression preserves nested braces", () => {
    const a = attrs("<div data={{ a: 1, b: [2, 3] }}/>")[0]!;
...
describe("if block", () => {
  test("simple if", () => {
    ...
    expect(block.branches[0]!.condition).toBe("ok");
  });

  test("if / else", () => {
    const block = only<IfBlock>("{#if ok}yes{:else}no{/if}");
    expect(block.branches.length).toBe(2);
    expect(block.branches[1]!.condition).toBe(null);
  });
  ...
  test("condition preserves nested braces", () => {
    ...
    expect(block.branches[0]!.condition).toBe("items[{k:1}.k]");
  });
});
...
describe("for block", () => {
  ...
  test("for await with catch", () => {
    ...
    expect(block.catch!.param).toBe("e");
  });
...
describe("await block", () => {
  ...
  test("await with then only", () => {
    ...
    expect(block.then!.param).toBe("value");
  ...
  test("inline `then` shorthand — body is the then branch, no pending", () => {
    ...
    expect(block.then!.param).toBe("value");
    expect(block.then!.children.some((n) => n.type === "Interpolation")).toBe(true);
    expect(block.catch).toBe(null);
  });
  ...
  test("inline `catch` shorthand — body is the catch branch, no then/pending", () => {
    ...
    expect(block.catch!.param).toBe("err");
  });
...
describe("switch block", () => {
  test("switch with cases and default", () => {
    const block = only<SwitchBlock>("{#switch status}{:case 'a'}A{:case 'b'}B{:default}D{/switch}");
...
describe("snippet block", () => {
  test("snippet with params", () => {
    const block = parse("{#snippet row(item, i)}<td>{item}</td>{/snippet}").children[0]!;
    expect(block.type).toBe("SnippetBlock");
    expect(block).toMatchObject({ name: "row", params: "item, i" });
  });

  test("snippet without params", () => {
    const block = parse("{#snippet header}<h1>hi</h1>{/snippet}").children[0]!;
...
describe("nesting", () => {
  test("if inside for inside element", () => {
    const root = parse("<ul>{#for item of items}{#if item.ok}<li>{item.name}</li>{/if}{/for}</ul>");
...
describe("script and style", () => {
  ...
  test("nested style inside a branch is not the root style", () => {
    ...
    expect(block.branches[0]!.children.some((n) => n.type === "Style")).toBe(true);
  });
...
describe("positions", () => {
  ...
  test("element span covers open through close tag", () => {
    ...
    const el = parse(source).children[0]!;
...
<section class="list">
  ...
  <Avatar {...avatarProps} />
    ...
    const section = root.children.find((n) => n.type === "Element" && n.name === "section") as Element;
