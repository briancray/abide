...
import type { DeferredStreamer, DeferredSubtree, StreamFrame, StreamHandleRecord, StreamScope } from "../../shared/internal/context.ts";
...
// `{#for await}` streaming budget (default 30s, `ABIDE_SSR_STREAM_BUDGET`) — the bound past which a
// still-running streamed list is cut off (client re-iterates) so an SSR `{#for await}` never hangs.
export function createStreamScope(): StreamScope {
  return {
    deadlinePassed: timerPromise(envMs("ABIDE_SSR_DEADLINE", 4), DEADLINE_PASSED),
    streamBudget: timerPromise(envMs("ABIDE_SSR_STREAM_BUDGET", 30_000), BUDGET_PASSED),
    deferred: [],
    streamers: [],
    streamHandles: [],
    nextId: 0,
  ...
  scope.deferred.push({
    ...
    render: async () => {
      ...
      try {
        ...
        console.error("[abide] streamed {#await} subtree threw with no {:catch} — cleared the slot:", error);
        return { html: "" };
      }
    },
  });
...
async function toIterator(expr: unknown): Promise<{ source: unknown; next: () => Promise<IteratorResult<unknown>> }> {
  const source = await (expr as Promise<unknown>);
  const obj = source as {
    [Symbol.asyncIterator]?: () => AsyncIterator<unknown>;
    [Symbol.iterator]?: () => Iterator<unknown>;
  } | null;
  if (obj != null && typeof obj[Symbol.asyncIterator] === "function") {
...
// ends within the budget. For an ATTACHABLE (known-RPC) source it ALSO captures the decoded item values
...
export async function forAwaitStream(config: ForAwaitStreamConfig): Promise<string> {
  ...
  for (;;) {
    const raced = await Promise.race([
      pending.then((step) => ({ kind: "item" as const, step }), (error) => ({ kind: "error" as const, error })),
    ...
    if (raced.step.done === true) {
      ...
      scope.streamHandles.push({ listId, name: config.rpcName ?? null, args: await resolveArgs(config), done: true, count: values.length, values });
  ...
  if (attachable) {
    handle = { listId, name: config.rpcName ?? null, args: await resolveArgs(config), done: false, count: values.length, values };
      ...
      try {
        for (;;) {
          const raced = await Promise.race([
            ...
            scope.streamBudget.then(() => ({ kind: "budget" as const })),
          ]);
...
export async function* drainPatches(scope: StreamScope): AsyncGenerator<Patch> {
  ...
  const kickSubtree = (subtree: DeferredSubtree): void => {
    ...
    inFlight.set(
      ...
      subtree.render().then((result) => ({
        key,
        patch: result === null ? null : ({ op: "fill", id: subtree.id, html: result.html } as Patch),
        advance: null,
      })),
    );
  };
  ...
  while (inFlight.size > 0) {
    ...
    for (const subtree of scope.deferred) if (!started.has(`s${subtree.id}`)) kickSubtree(subtree);
    for (const streamer of scope.streamers) if (!started.has(`l${streamer.id}`)) kickStreamer(streamer);
  }
}
