...
import { loadEmitted, loadEmittedServer, emitModuleSource } from "./emit.ts";
...
async function mount(source: string, scope: Record<string, unknown> = {}): Promise<{ host: HTMLElement; dispose: () => void }> {
...
function reactiveScope(entries: Record<string, ReturnType<typeof signal>>): Record<string, unknown> {
...
describe("fine-grained reactivity", () => {
  test("only the affected text node updates (identity preserved)", async () => {
    ...
    const spanB = host.querySelectorAll("span")[1]!;
    const textBefore = spanB.firstChild;
    a.set("A");
    await tick();
    expect(host.querySelectorAll("span")[0]!.textContent).toBe("A");
    expect(host.querySelectorAll("span")[1]!.firstChild).toBe(textBefore);
  });
  ...
  test("class:name toggles reactively", async () => {
    ...
    expect(host.querySelector("div")!.classList.contains("on")).toBe(false);
    active.set(true);
    await tick();
    expect(host.querySelector("div")!.classList.contains("on")).toBe(true);
  });

  test("style:prop sets a property reactively", async () => {
    const color = signal("red");
    const { host } = await mount("<div style:color={color}></div>", reactiveScope({ color }));
    expect(host.querySelector("div")!.style.color).toBe("red");
    color.set("blue");
    await tick();
    expect(host.querySelector("div")!.style.color).toBe("blue");
  });
  ...
  test("{html()} updates reactively", async () => {
    ...
    expect(host.querySelector("u")!.textContent).toBe("b");
  });
});
...
describe("two-way binding", () => {
  ...
  test("bind:value over a bare state cell synthesizes a get/set accessor (client + server) (#14)", () => {
    ...
    const out = emitModuleSource("<script>import { state } from 'abide/ui/state'; let bare = state('direct')</script><input bind:value={bare}>");
  ...
  test("bind:value with a derived {get,set}", async () => {
    const raw = signal("x");
    const derived = { get: () => raw().toUpperCase(), set: (v: unknown) => raw.set(String(v).toLowerCase()) };
...
describe("bind:element", () => {
  ...
  test("assigns the node to a bare state() cell (node ref)", async () => {
    ...
    const { host } = await mount("<script>import { state } from 'abide/ui/state'\nlet node = state(null)</script><input bind:element={node}><p>{node ? node.tagName : 'none'}</p>", { state });
...
describe("{#for} keyed reconciliation", () => {
  test("keyed add and remove", async () => {
    ...
    const { host } = await mount("{#for n of items by n}<li>{n}</li>{/for}", reactiveScope({ items }));
  ...
  test("keyed reuse preserves the survivor node on prepend", async () => {
    ...
    const { host } = await mount("{#for item of items by item.id}<li>{item.id}</li>{/for}", reactiveScope({ items }));
...
describe("no ambient identifiers (script)", () => {
  ...
  test("state() without an import does not resolve", async () => {
    const emitted = await loadEmitted("<script>let count = state(0)</script><span>{count}</span>");
...
describe("contextual-keyword template identifiers (#18)", () => {
  ...
  test("safe keyword-named references resolve via $scope", async () => {
    const { host } = await mount("<p>{accessor}|{type}|{object}</p>", { accessor: "AX", type: "TY", object: "OB" });
    expect(host.querySelector("p")!.textContent).toBe("AX|TY|OB");
  });
  ...
  test("dangerous keywords keep their operator meaning — {await fn()} is not rewritten", () => {
    const out = emitModuleSource("<script>import greet from '../rpc/greet'</script><p>{await greet()}</p>");
...
describe("server-side bind serialization (regression: accessor not unwrapped)", () => {
  ...
  test("bind:checked reflects the accessor's boolean, not object truthiness", async () => {
    ...
    expect(await off.render({ acc: { get: () => false, set: () => {} } })).toBe('<input type="checkbox">');
    const on = await loadEmitted('<input type="checkbox" bind:checked={acc}>');
    expect(await on.render({ acc: { get: () => true, set: () => {} } })).toBe('<input type="checkbox" checked>');
  });
  ...
  test("bind:value serializes the resolved value, not the accessor object", async () => {
    ...
    expect(await emitted.render({ acc: { get: () => "hello", set: () => {} } })).toBe('<input value="hello">');
  });
...
describe("<script module> bindings reach the template (regression: docs app SSR)", () => {
  ...
  test("module and instance importing the SAME name do not double-declare (const collision)", async () => {
    ...
    expect(stripAnchors(await emitted.render({ state: (v: unknown) => ({ read: () => v }) }))).toBe("<p>MOD/INST</p>");
  });
});
...
describe("attribute-value interpolation", () => {
  ...
  test("a component prop interpolates too (label=\"…{x}…\")", async () => {
    ...
    const parent =
      ...
      "<Badge label=\"n is {n}\" />";
    const resolve = (specifier: string): string | undefined => (specifier === "./Badge.abide" ? child : undefined);
...
describe("M3b pass-through framework imports", () => {
  ...
  test("server render executes the real import — online() is true on the server", async () => {
    ...
    const html = await mod.render({ state: (v: unknown) => ({ read: () => v, write: () => {}, peek: () => v }) });
