...
async function ssrStream(source: string, serverScope: Record<string, unknown>): Promise<{ html: string; seed: HydrationSeed }> {
  const emitted = await loadEmitted(source);
  const ctx = createContext();
  ctx.stream = createStreamScope();
  let html = "";
  await runInContext(ctx, async () => {
    ...
    for await (const _patch of drainPatches(ctx.stream!)) void _patch;
  });
...
describe("mode A — completed RPC {#for await} adopts the seeded transcript (no client re-run)", () => {
  ...
  test("the RPC source is never invoked on the client; items render + an item onclick fires", async () => {
    ...
    expect(seed.streams![0]!.values).toEqual(["t0", "t1", "t2"]);
...
describe("mode B — an OPEN RPC {#for await} resumes over ?from=<count> (no client re-run)", () => {
  ...
  test("adopts the flushed prefix, then resumes the remainder over the resumable replay endpoint", async () => {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL): Promise<Response> => {
      ...
      const body = new ReadableStream<Uint8Array>({
        start(controller) {
          ...
          for (const value of ["t2", "t3", "t4"]) controller.enqueue(encoder.encode(JSON.stringify(value) + "\n"));
          controller.close();
        },
      });
      return new Response(body, { status: 200, headers: { "x-abide-stream-resume": "live", "content-type": "application/jsonl" } });
    ...
    try {
      ...
      const lis = host.querySelectorAll("li");
      expect(Array.from(lis).map((li) => li.textContent)).toEqual(["t0", "t1", "t2", "t3", "t4"]);
