...
test("read proxy fetches and returns the handler value", async () => {
  ...
  const greet = clientProxy<{ name: string }, string>("greet", "GET", { base: app.origin }) as Rpc<{ name: string }, string>;
...
test("mutation proxy posts a JSON body and returns the value", async () => {
  ...
  const bump = clientProxy<{ n: number }, { next: number }>("bump", "POST", { base: app.origin }) as Mutation<{ n: number }, { next: number }>;
...
test("read proxy throws HttpError-like on non-2xx (404 unknown rpc)", async () => {
  ...
  const missing = clientProxy<{}, string>("nope", "GET", { base: app.origin }) as Rpc<{}, string>;
...
test("read proxy throws on 422 validation failure", async () => {
  const app = boot({
    greet: GET((args: { name: string }) => `hello ${args.name}`, {
      schemas: { input: { type: "object", properties: { name: { type: "string" } }, required: ["name"] } },
    }),
  });
  const greet = clientProxy<{ name?: string }, string>("greet", "GET", { base: app.origin }) as Rpc<{ name?: string }, string>;
