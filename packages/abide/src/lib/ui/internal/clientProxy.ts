...
import type { Rpc, Mutation } from "../../server/internal/makeRpc.ts";
...
class HttpErrorLike extends Error {
  ...
  constructor(status: number, statusText: string, message: string, kind?: string, data?: unknown) {
    super(message);
    this.name = "HttpError";
    this.status = status;
...
async function toHttpError(response: Response): Promise<HttpErrorLike> {
  ...
  const message = typeof body?.message === "string" ? body.message : statusText || `HTTP ${status}`;
...
export function clientProxy<Args = unknown, T = unknown>(
  ...
  if (isRead(method)) {
    ...
    const ensureSubscribed = (args: Args): void => {
      ...
      subscribed.add(key);
      subscribeCacheChannel(cacheChannelName(name, args), args, (frame) => applyCacheFrame(backing, args, frame), base);
    };
    ...
    rpc.raw = (args: Args, init?: RequestInit): Promise<Response> => fetch(readUrl(base, name, args), { method, ...(init ?? {}) });
    rpc.isError = (e: unknown, name: string): boolean =>
      e !== null && typeof e === "object" && ((e as Record<string, unknown>).kind === name || (e as Record<string, unknown>).name === name);
  ...
  const mutation = (async (args: Args | FormData): Promise<T> => {
    ...
    const response = await fetch(`${base}/rpc/${name}`, {
      method,
      headers: isFormData ? { "x-abide": "1" } : { "content-type": "application/json", "x-abide": "1" },
...
export function makeClientImports(
  ...
  for (const name of Object.keys(specs)) {
    ...
    imports[name] = clientProxy(name, spec.method, { base: base ?? "", shared: spec.shared === true });
  }
  return imports;
}
