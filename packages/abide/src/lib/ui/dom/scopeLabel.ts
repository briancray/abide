...
into `resolve`; any other host yields its lowercased tag. A human-readable scope name
for dev tooling — currently unconsumed (the inspector's Reactive tab that read it was
removed); the label plumbing is retained pending its own cleanup. Returns undefined when
there's no element to name from.
*/
