...
patch — no reader would re-render and undo/persistence/multiplayer sync (all keyed
off `apply`) would never see the change (`.push` is the one already-handled
exception, lowered to `add` patches). Cloning the container, applying the mutation
to the copy, and writing it back through `replace` emits a real patch: readers wake
and the change is journalled.
