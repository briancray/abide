...
import { validateStandard, type StandardSchemaV1 } from "../shared/StandardSchema.ts";
...
const outputMustBeNumberSchema: StandardSchemaV1<number, number> = {
  "~standard": {
    ...
    validate(value: unknown): StandardSchemaV1.Result<number> {
      if (typeof value !== "number") return { issues: [{ message: "output must be a number", path: [] }] };
      return { value };
    },
  },
};
...
describe("validateStandard + ValidationErrorData (unit)", () => {
  ...
  test("validationError builds a 422 with kind ValidationError and data", async () => {
    ...
    const body = (await response.json()) as { status: number; kind: string; data: { fields: Record<string, string> } };
...
describe("RPC input validation (integration)", () => {
  test("valid GET args run the handler and return 200", async () => {
    ...
    try {
      const response = await app.fetch(`/rpc/read?args=${encodeURIComponent(JSON.stringify({ id: 5 }))}`);
  ...
  test("invalid GET args return 422 ValidationError and never call the handler", async () => {
    ...
    try {
      const response = await app.fetch(`/rpc/read?args=${encodeURIComponent(JSON.stringify({ id: "bad" }))}`);
      expect(response.status).toBe(422);
      const body = (await response.json()) as { kind: string; data: { fields: Record<string, string> } };
      expect(body.kind).toBe("ValidationError");
      expect(body.data.fields.id).toBe("id must be a number");
      expect(calls).toBe(0);
    } finally {
  ...
  test("valid POST args run the handler; invalid args return 422 without calling it", async () => {
    ...
    try {
      ...
      const body = (await bad.json()) as { kind: string; data: { fields: Record<string, string> } };
      expect(body.kind).toBe("ValidationError");
      expect(body.data.fields.id).toBe("id must be a number");
      expect(calls).toBe(1);
    } finally {
...
describe("RPC input validation with a derived JSON Schema (integration)", () => {
  ...
  test("a JSON Schema derived from a handler drives 200/422 through the RPC path", async () => {
    ...
    try {
      ...
      const ok = await app.fetch(`/rpc/echo?args=${encodeURIComponent(JSON.stringify({ text: "hi" }))}`);
      expect(ok.status).toBe(200);
      expect(await ok.json()).toEqual({ echoed: "hi" });
      expect(calls).toBe(1);

      // Wrong type for `text` → 422 ValidationError, handler never runs.
      const badType = await app.fetch(`/rpc/echo?args=${encodeURIComponent(JSON.stringify({ text: 123 }))}`);
      expect(badType.status).toBe(422);
      const typeBody = (await badType.json()) as { kind: string; data: { fields: Record<string, string> } };
      ...
      const missing = await app.fetch(`/rpc/echo?args=${encodeURIComponent(JSON.stringify({}))}`);
      expect(missing.status).toBe(422);
      const missingBody = (await missing.json()) as { kind: string; data: { fields: Record<string, string> } };
...
describe("RPC output validation (dev-only)", () => {
  test("output-schema mismatch logs a warning but still returns 200 in dev", async () => {
    ...
    const app = createTestApp({
      routes: {
        drift: GET((): unknown => "not a number", { schemas: { output: outputMustBeNumberSchema } }),
      },
    });
    try {
      ...
      expect(warnings.some((entry) => String(entry[0]).includes("output schema mismatch"))).toBe(true);
    } finally {
