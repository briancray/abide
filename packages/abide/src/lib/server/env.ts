...
import { asStandardSchema, type JSONSchema, type JSONSchemaType } from "../shared/internal/jsonSchema.ts";
...
type EnvFieldPresent<F extends EnvFieldSpec> = F extends { default: unknown } ? true : F extends { required: true } ? true : false;
...
export function env<T = Record<string, unknown>>(schema?: EnvSchema): T {
  ...
  const standard = asStandardSchema(isStandard ? (schema as StandardSchemaV1) : (schema as JSONSchema));
  ...
  if (validation.issues !== undefined) {
    const details = validation.issues.map((issue) => {
      const path = issue.path?.map((segment) => (typeof segment === "object" ? String(segment.key) : String(segment))).join(".");
...
function singleType(type: JSONSchemaType | JSONSchemaType[] | undefined): JSONSchemaType | undefined {
...
function coerce(raw: string, field: Field): Coerced {
  ...
  switch (field.type) {
    ...
    case "integer": {
      ...
      if (!Number.isFinite(parsed) || raw.trim() === "") return { ok: false, message: `expected a ${field.type}, got "${raw}"` };
      if (field.type === "integer" && !Number.isInteger(parsed)) return { ok: false, message: `expected an integer, got "${raw}"` };
      value = parsed;
      break;
    }
    case "boolean": {
  ...
  if (field.enumValues !== undefined && !field.enumValues.includes(value)) {
    return { ok: false, message: `expected one of ${field.enumValues.map((entry) => JSON.stringify(entry)).join(", ")}, got "${raw}"` };
  }
...
function formatFailure(missing: string[], invalid: string[]): string {
  const parts: string[] = [];
  if (missing.length > 0) parts.push(`missing required environment variable(s): ${missing.join(", ")}`);
