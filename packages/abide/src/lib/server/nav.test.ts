...
test("SSRs a param route, filling route().params from the pathname", async () => {
  const app = createTestApp({
    pages: { "/users/[id]": "<script>import { route } from 'abide/shared/route'</script><span>{route().params.id}</span>" },
  });
...
test("an exact route beats a param route when both match", async () => {
  const app = createTestApp({
    pages: {
      "/users/[id]": "<script>import { route } from 'abide/shared/route'</script><span>param:{route().params.id}</span>",
