...
describe('agent approval gate', () => {
    test('approve → tool runs and the result lands in the transcript', async () => {
        ...
        if (secondTurn === undefined) throw new Error('expected a second turn')
    ...
    test('deny → tool does NOT run, transcript carries a denied tool-result the model sees', async () => {
        ...
        const approval: ApprovalPolicy = {
            required: true,
            decide: async (): Promise<ApprovalDecision> => ({
                action: 'deny',
                reason: 'not allowed',
            }),
        }
        ...
        const toolResult = frames.find((f) => f.type === 'tool-result') as
            | { error?: unknown }
            | undefined
        expect(toolResult).toBeDefined()
        if (!toolResult) throw new Error('expected a tool-result frame')
        ...
        if (toolMessage === undefined) throw new Error('expected a tool message')
        expect(toolMessage.role).toBe('tool')
        const part = (toolMessage.content as { type: string; error?: unknown }[])[0]
        if (part === undefined) throw new Error('expected a content part')
    ...
    test('required predicate → only the named tool is gated, others auto-run', async () => {
        ...
        const frames = await collect(
            agent(engine, [user('go')], { tools: [gated, free], approval }),
        )
    ...
    test('abort while awaiting a decision → the loop returns cleanly', async () => {
        ...
        const frames = await collect(
            agent(engine, [user('go')], { tools: [tool], approval, signal: controller.signal }),
        )
        ...
        const approver = (async (): Promise<void> => {
            while (true) {
                ...
                if (frame.kind === 'request') {
                    publishApprovalDecision('run-1', 'call-a', {
                        action: 'edit',
                        args: { edited: true },
                    })
                    return
