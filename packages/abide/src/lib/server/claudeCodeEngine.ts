...
async function* streamTurn(
    ...
    try {
        ...
        yield {
            type: 'error',
            error: new Error(
                `claudeCodeEngine: could not spawn "${opts.path ?? 'claude'}" — is Claude Code installed? (${errorText(caught)})`,
            ),
        }
    ...
    try {
        ...
        if (signal?.aborted !== true) {
            ...
            if (!sawResult && !emittedError && code !== 0) {
                yield {
                    type: 'error',
                    error: new Error(`claudeCodeEngine: claude exited ${code} without a result`),
                }
            }
        }
    } finally {
...
function translate(line: StreamJsonLine, toolNames: Map<string, string>): AgentFrame[] {
    ...
    switch (line.type) {
        ...
        case RESULT_TYPE: {
            ...
            if (
                line.is_error === true ||
                (line.subtype !== undefined && line.subtype !== 'success')
            ) {
                out.push({
                    type: 'error',
                    error: new Error(
                        `claude result ${line.subtype ?? 'error'}: ${stringify(line.result)}`,
                    ),
                })
            }
            break
...
function buildCommand(
    ...
    if (opts.allowedTools !== undefined && opts.allowedTools.length > 0)
        command.push('--allowedTools', ...opts.allowedTools)
    if (opts.disallowedTools !== undefined && opts.disallowedTools.length > 0)
        command.push('--disallowedTools', ...opts.disallowedTools)
...
function renderPrompt(
    ...
    let prompt: string
    const only = conversation.length === 1 ? conversation[0] : undefined
    if (only !== undefined && only.role === 'user') {
        prompt = collectText(only.content)
    } else {
        prompt = conversation
            .map((message) => `[${message.role}] ${collectText(message.content)}`)
            .join('\n\n')
    }
