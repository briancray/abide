...
describe("multipart RPC upload (TODO #8)", () => {
  test("handler receives the FormData: text field + uploaded file", async () => {
    ...
    try {
      ...
      const result = (await app.rpc.upload!(form)) as { field: string; name: string; content: string; size: number };
  ...
  test("a plain Blob is received as a File on the server", async () => {
    ...
    try {
      ...
      const result = (await app.rpc.upload!(form)) as { content: string; size: number };
  ...
  describe("CSRF gate for multipart", () => {
    ...
    test("multipart mutation WITH x-abide is admitted (200)", async () => {
      ...
      try {
        ...
        form.set("caption", "x");
        form.set("avatar", new File(["y"], "y.txt"));
        const response = await app.fetch("/rpc/upload", { method: "POST", headers: { "x-abide": "1" }, body: form });
  ...
  describe("files schema validation", () => {
    const guardedUpload = POST(async (form: FormData) => ({ ok: form.get("avatar") instanceof File }), {
      schemas: { files: { required: ["avatar"], properties: { avatar: { accept: "text/*", maxSize: 1024 } } } },
    });
    ...
    test("required file present + within constraints → 200", async () => {
      ...
      try {
        ...
        const result = (await app.rpc.upload!(form)) as { ok: boolean };
  ...
  describe("text field validation (input schema, TODO #8 follow-up)", () => {
    ...
    const captioned = POST(async (form: FormData) => ({ caption: String(form.get("caption")), count: form.get("count") }), {
      schemas: {
        input: { type: "object", properties: { caption: { type: "string" }, count: { type: "number" } }, required: ["caption"] },
    ...
    test("valid text + file → 200 (handler still receives raw FormData)", async () => {
      ...
      try {
        ...
        form.set("count", "3");
        form.set("avatar", new File(["y"], "y.txt"));
        const result = (await app.rpc.upload!(form)) as { caption: string; count: unknown };
    ...
    test("a coercible number-string passes, a non-number 422s (declared {type:number})", async () => {
      ...
      try {
        ...
        expect((await app.fetch("/rpc/upload", { method: "POST", headers: { "x-abide": "1" }, body: ok })).status).toBe(200);

        const bad = new FormData();
        bad.set("caption", "c");
        bad.set("count", "not-a-number");
        bad.set("avatar", new File(["y"], "y.txt"));
        expect((await app.fetch("/rpc/upload", { method: "POST", headers: { "x-abide": "1" }, body: bad })).status).toBe(422);
      } finally {
        await app.stop();
      }
    });
  ...
  test("maxBodySize rejects an oversize multipart body (413)", async () => {
    const bounded = POST(async (form: FormData) => ({ got: form.get("avatar") instanceof File }), { maxBodySize: 32 });
  ...
  test("existing JSON mutations are unaffected (back-compat)", async () => {
    ...
    try {
      const result = (await app.rpc.echo!({ value: 21 })) as { doubled: number };
