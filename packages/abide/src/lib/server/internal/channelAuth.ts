...
import { runInScope, type Principal, type RequestScope } from "./scope.ts";
...
export async function authorizeChannelJoin(
  ...
  const syntheticRequest = new Request(rpcUrl, { method: "GET", headers: connData.request.headers });
  const scope: RequestScope = {
    ...
    route: {
      kind: "rpc",
      name: rpcName,
      params: presentedArgs !== null && typeof presentedArgs === "object" ? (presentedArgs as Record<string, unknown>) : {},
      url: rpcUrl,
      navigating: false,
    },
