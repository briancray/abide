...
import { runInScope, type RequestScope, type RouteInfo, type RouteKind } from "./scope.ts";
...
function csrfReject(request: Request): Response | undefined {
  ...
  if (!hasNonSimpleShape) {
    return error(403, "CSRF: mutations require Content-Type: application/json or an x-abide header.");
  }
...
async function applyIdentityCookie(scope: RequestScope, response: Response): Promise<void> {
  ...
  const header = scope.identityCleared ? clearIdentityCookieHeader() : await identityCookieHeader(scope.identity);
...
async function pumpSocketToWs(ws: Bun.ServerWebSocket<SocketConnectionData>, connection: SocketConnection, name: string, iterator: AsyncIterator<unknown>): Promise<void> {
  try {
    while (true) {
...
function wsSubscribe(ws: Bun.ServerWebSocket<SocketConnectionData>, connection: SocketConnection, name: unknown, args: unknown, sockets: Record<string, Socket<any>>, config: AppConfig): void {
...
async function subscribeCacheChannel(ws: Bun.ServerWebSocket<SocketConnectionData>, connection: SocketConnection, name: string, args: unknown, config: AppConfig): Promise<void> {
...
async function wsPublish(name: unknown, message: unknown, sockets: Record<string, Socket<any>>): Promise<void> {
  if (typeof name !== "string") return;
  const sock = sockets[name];
  if (sock === undefined) return;
...
async function socketHttpFace(request: Request, url: URL, sockets: Record<string, Socket<any>>): Promise<Response> {
...
async function dispatch(scope: RequestScope, config: AppConfig): Promise<Response> {
  ...
  if (url.pathname === "/__abide/client.js") {
    ...
    if (method !== "GET" && method !== "HEAD") return error(405, `Method not allowed: ${method}`);
    const bundle = await buildClientBundle(config);
    return new Response(bundle, { status: 200, headers: { "content-type": "text/javascript; charset=utf-8" } });
  }
      ...
      try {
        ...
        if (isSoftNav(scope.request)) {
          ...
          return new Response(body, {
            status: 200,
            headers: { "content-type": "application/jsonl; charset=utf-8", vary: "Abide-Nav" },
          });
        }
  ...
  if (fromRaw !== null && /^\d+$/.test(fromRaw)) {
    const resumable = route as Rpc<any, any> & { resumeStream(a: unknown, f: number): { cursor: AsyncIterable<unknown> | undefined; fresh: boolean } };
  ...
  const result = meta.read ? await (route as Rpc<any, any>).load(args) : await (route as Mutation<any, any>)(args);
  ...
  if (isAsyncIterable(result)) {
    ...
    const useSse = encoding === "sse" || (encoding === undefined && accept.includes("text/event-stream"));
  ...
  if (outputSchema !== undefined && Bun.env.NODE_ENV !== "production") {
    ...
    if (!checked.ok) {
      console.warn(`[abide] output schema mismatch for rpc "${scope.route.name}":`, checked.issues);
    }
  }
  ...
  const server = Bun.serve<SocketConnectionData>({
    ...
    async fetch(request, srv): Promise<Response | undefined> {
      ...
      if (url.pathname === "/__abide/sockets") {
        ...
        const connData: SocketConnectionData = { request, identity: await resolveIdentity(request) };
      ...
      const route: RouteInfo = { kind: info.kind, name: info.name, params: {}, url, navigating: false };
      ...
      const propagatedTrace = incomingTrace !== null && TRACEPARENT_PATTERN.test(incomingTrace) ? incomingTrace : undefined;
      ...
      const chain = compose([...globalMiddleware, ...rpcMiddleware], () => dispatch(scope, config));
      ...
      return runInScope(scope, async () => {
        ...
        if (scope.traceparent !== undefined) response.headers.set("traceparent", scope.traceparent);
        return response;
      }) as Promise<Response>;
    },
    websocket: {
      ...
      message(ws, raw): void {
        ...
        if (frame.t === "sub") wsSubscribe(ws, connection, frame.name, frame.args, sockets, config);
