...
function listTools(config: AppConfig): McpTool[] {
  ...
  for (const rpc of registry.rpcs) {
    ...
    const tool: McpTool = {
      name: rpc.name,
      inputSchema: (rpc.inputSchema as Record<string, unknown> | undefined) ?? { type: "object" },
...
async function callTool(name: string, args: unknown, config: AppConfig, request: Request): Promise<Outcome> {
  const registry = buildRegistry(config);
  ...
  for (const rpc of registry.rpcs) {
    ...
    if (rpc.name === name) {
      try {
        ...
        return { error: { code: INTERNAL_ERROR, message: caught instanceof Error ? caught.message : "tool call failed" } };
      }
    }
  }
...
async function callSocketTool(name: string, args: unknown, entries: SocketEntry[], sockets: Record<string, Socket<any>>): Promise<Outcome | undefined> {
  for (const entry of entries) {
    ...
    if (entry.clientPublish && name === `${entry.name}_publish`) {
      try {
        ...
        return { result: textResult(caught instanceof Error ? caught.message : "publish rejected", true) };
      }
    }
  }
  return undefined;
...
async function handleMessage(message: JsonRpcRequest, config: AppConfig, request: Request): Promise<Outcome | null> {
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      if (typeof params.name !== "string") {
        return { error: { code: INVALID_PARAMS, message: "tools/call requires a string `name`." } };
      }
...
export async function handleMcp(request: Request, config: AppConfig): Promise<Response> {
  ...
  try {
    ...
    return json({ jsonrpc: "2.0", id: null, error: { code: PARSE_ERROR, message: "Parse error." } });
  }
