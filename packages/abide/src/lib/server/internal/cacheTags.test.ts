...
import { cacheChannelName, cacheChannelHub, tagChannelName, type CacheFrame } from "./cacheChannels.ts";
import { publishCacheFrame } from "./cacheChannels.ts";
...
describe("cache tags — global invalidate({ tags })", () => {
  test("drops the slots of every shared read carrying the tag and broadcasts on each @rpc: channel", async () => {
    let callsX = 0;
    let callsY = 0;
    const readX = makeRead("GET", async ({ id }: { id: number }) => { callsX++; return { id, v: "x" }; }, { cache: { shared: true, tags: ["user"] } });
    const readY = makeRead("GET", async ({ id }: { id: number }) => { callsY++; return { id, v: "y" }; }, { cache: { shared: true, tags: ["user"] } });
  ...
  test("a cell tagged 'a' is NOT affected by invalidate({ tags: ['b'] })", async () => {
    let calls = 0;
    const read = makeRead("GET", async ({ id }: { id: number }) => { calls++; return id; }, { cache: { shared: true, tags: ["a"] } });
  ...
  test("multiple tags on one cell — a partial tag match still selects it", async () => {
    let calls = 0;
    const read = makeRead("GET", async ({ id }: { id: number }) => { calls++; return id; }, { cache: { shared: true, tags: ["user", "org"] } });
  ...
  test("selects each cell once even when it carries several listed tags", async () => {
    const read = makeRead("GET", async ({ id }: { id: number }) => id, { cache: { shared: true, tags: ["a", "b"] } });
...
describe("cache tags — global refresh({ tags })", () => {
  test("eagerly revalidates tagged slots and broadcasts refresh frames", async () => {
    let calls = 0;
    const read = makeRead("GET", async ({ id }: { id: number }) => { calls++; return { id, calls }; }, { cache: { shared: true, tags: ["user"] } });
...
describe("cache tags — @tag channel", () => {
  test("invalidate({ tags }) emits a frame on the @tag:<tag> channel", async () => {
    const read = makeRead("GET", async ({ id }: { id: number }) => id, { cache: { shared: true, tags: ["user"] } });
...
describe("cache tags — local reactive probes", () => {
  test("pending({ tags }) / refreshing({ tags }) aggregate over tagged slots", async () => {
    const read = makeRead("GET", async ({ id }: { id: number }) => { await new Promise((r) => setTimeout(r, 20)); return id; }, { cache: { shared: true, tags: ["user"] } });
