...
export async function resolveIdentity(request: Request): Promise<Principal> {
  ...
  if (authorization !== null) {
    ...
    if (match !== null) {
      ...
      if (appToken !== undefined && appToken.length > 0 && constantTimeEqual(bearer, appToken)) {
        return appOwner();
      }
...
export function requireSecretForAuthedSet(authenticated: boolean): void {
  ...
  if (secret === undefined || secret.length === 0) {
    throw new Error("abide: cannot set an authenticated identity in production without ABIDE_IDENTITY_SECRET — an ephemeral key would produce a forgeable identity that resets on every restart. Set ABIDE_IDENTITY_SECRET.");
  }
}
