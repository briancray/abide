...
describe("projectFormText", () => {
  ...
  test("an opaque Standard Schema yields raw strings (no framework coercion)", () => {
    const standard = { "~standard": { version: 1, vendor: "test", validate: (v: unknown) => ({ value: v }) } };
    const form = new FormData();
    form.set("count", "42");
    // biome-ignore lint/suspicious/noExplicitAny: exercising the Standard-Schema branch
    const out = projectFormText(form, standard as any);
