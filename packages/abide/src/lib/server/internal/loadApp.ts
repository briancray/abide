...
async function loadPages(dir: string): Promise<{ pages: Record<string, string>; dirs: Record<string, string> }> {
...
async function loadLayouts(dir: string): Promise<{ layouts: Record<string, string>; dirs: Record<string, string> }> {
  const pagesDir = join(dir, "src/ui/pages");
...
async function loadAppModule(dir: string): Promise<{ middleware: Middleware[]; lifecycle: AppLifecycle }> {
  ...
  if (typeof module.onStart === "function") lifecycle.onStart = module.onStart as () => void | Promise<void>;
  if (typeof module.onStop === "function") lifecycle.onStop = module.onStop as () => void | Promise<void>;
  if (typeof module.health === "function") lifecycle.health = module.health as () => unknown | Promise<unknown>;
