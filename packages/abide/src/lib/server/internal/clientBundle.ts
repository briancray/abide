...
function rpcSpecs(config: AppConfig, importedNames: Set<string>): Record<string, { method: string; read: boolean; shared: boolean }> {
...
function resolveCssImports(client: string, cssImports: string[], sourceDir: string | undefined): string {
  if (sourceDir === undefined) return client;
  let out = client;
  for (const specifier of cssImports) {
    if (specifier.startsWith("./") || specifier.startsWith("../")) {
      const absolute = join(sourceDir, specifier);
      out = out.replace(`import ${JSON.stringify(specifier)};`, `import ${JSON.stringify(absolute)};`);
    }
  }
  return out;
}
...
async function emitOne(
  ...
  let client = emitModuleSource(source).client.replace('"abide/ui/internal/runtime"', JSON.stringify(RUNTIME_PATH));
  ...
  for (const componentImport of analysis.componentImports) {
    if (sourceDir === undefined) {
      throw new Error(`abide: <${componentImport.local}> imports "${componentImport.specifier}" but the importer's source dir is unknown (needed to resolve .abide components in the client bundle).`);
    }
    const childPath = join(sourceDir, componentImport.specifier);
    const childSource = await Bun.file(childPath).text();
    const childIndex = await emitOne(childSource, dirname(childPath), visited, modules, childPath);
    client = client.replaceAll(`from ${JSON.stringify(componentImport.specifier)}`, `from ${JSON.stringify(modules[childIndex]!.file)}`);
  }
...
async function emitModules(config: AppConfig): Promise<{ modules: EmittedModule[]; chains: PageChain[] }> {
  ...
  for (const pattern of Object.keys(pages)) {
    ...
    for (const prefix of applicableLayoutPrefixes(pattern, layouts)) {
      indices.push(await emitOne(layouts[prefix]!, layoutDirs[prefix], visited, modules));
    }
    indices.push(await emitOne(pages[pattern]!, pageDirs[pattern], visited, modules));
...
function entrySource(modules: EmittedModule[], chains: PageChain[], specsJson: string): string {
  ...
  for (let i = 0; i < modules.length; i++) {
    ...
    imports += `import { mount as $m${i}, hydrate as $h${i} } from ${JSON.stringify(modules[i]!.file)};\n`;
...
async function build(config: AppConfig): Promise<ClientArtifacts> {
  ...
  try {
    ...
    for (const output of result.outputs) {
      ...
      else if (output.kind === "entry-point" || (js === "" && !output.path.endsWith(".css"))) js = await output.text();
    }
