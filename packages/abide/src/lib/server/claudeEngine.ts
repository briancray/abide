...
export function claudeEngine(opts: { apiKey?: string; model?: string; baseUrl?: string } = {}): AgentEngine {
  return {
...
async function* streamTurn(
  ...
  if (!response.ok || response.body === null) {
    ...
    yield { type: "error", error: new Error(`Anthropic API error ${response.status}: ${detail}`) };
    yield { type: "message-stop" };
    return;
  }
      ...
      case "content_block_stop": {
        if (event.index !== undefined) {
          ...
          if (pending !== undefined) {
            ...
            yield { type: "tool-call", id: pending.id, name: pending.name, args: parseToolArgs(pending.json) };
          }
        }
        break;
      }
...
function toolResultBlock(part: { type: "tool-result"; id: string; result: unknown; error?: unknown }): AnthropicBlock {
  if (part.error !== undefined) {
    return { type: "tool_result", tool_use_id: part.id, content: stringify(part.error), is_error: true };
  }
