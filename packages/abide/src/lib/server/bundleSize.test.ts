...
test("the served client bundle contains no TypeScript compiler and is small", async () => {
  const app = createTestApp({
    ...
    pages: { "/": "<script>import { state } from 'abide/ui/state'; let count = state(0)</script><button onclick={() => count++}>+</button><span>{count}</span>" },
  });
  ...
  // — replayable-streams.md §4. FUTURE: the byte-accounting/pin/cap path is server-only and could be
  // extracted out of the isomorphic cell to shrink the client bundle back down.)
  const bytes = Buffer.byteLength(body, "utf8");
  expect(bytes).toBeLessThan(62_000);
...
test("bootstrapPage HYDRATES via the TS7-free path (claims server nodes, working reactivity)", async () => {
  const source = "<script>import { state } from 'abide/ui/state'; let count = state(0)</script><button onclick={() => count++}>+</button><span>{count}</span>";
