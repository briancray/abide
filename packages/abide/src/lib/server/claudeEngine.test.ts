...
describe("claudeEngine", () => {
  ...
  test("normalizes a canned Anthropic SSE stream into frames", async () => {
    ...
    expect(toolCall).toEqual({ type: "tool-call", id: "toolu_1", name: "get_weather", args: { city: "Paris" } });
  ...
  test("agent() drives a full two-turn tool loop to done", async () => {
    ...
    const frames = await collect(agent(claudeEngine(), [user("weather in Paris?")], { tools: [tool] }));
