...
describe("loadApp — file-based app loader", () => {
  test("discovers rpc route names, page paths, and the app middleware export", async () => {
    ...
    expect(Object.keys(loaded.routes ?? {})).toEqual(["greet"]);
    expect(loaded.routes!.greet!.__rpc.read).toBe(true);

    expect(Object.keys(loaded.pages ?? {}).sort()).toEqual(["/", "/about"]);
    expect(loaded.pages!["/"]).toContain("greet");

    // TODO #7: layout.abide files are discovered and keyed by their directory route prefix.
    expect(Object.keys(loaded.layouts ?? {})).toEqual(["/"]);
    expect(loaded.layouts!["/"]).toContain("children()");
  ...
  test("the loaded config boots a working app: rpc + SSR pages", async () => {
    ...
    expect(await app.rpc.greet!({ name: "world" })).toBe("hi world");
    ...
    expect(stripped.indexOf('<div class="app">')).toBeLessThan(stripped.indexOf("<h1>hi x</h1>"));
