// abide's native macOS shim, compiled into the same dylib as the vendored
...
// Exported despite -fvisibility=hidden so abide's FFI layer can resolve it.
#define ABIDE_EXPORT __attribute__((visibility("default")))

/*
Process-wide connection flag the menu validation reads. The launcher owns every
connection transition and flips this via abide_set_connected, so the Server menu
...
*/
static int g_abide_connected = 0;
...
static void abideDispatchNavigate(void *w, void *arg) {
    char *url = (char *)arg;
...
gone — abideDispatchNavigate frees the copy once it has navigated.
*/
extern "C" ABIDE_EXPORT void abide_request_navigate(void *w, const char *url) {
    webview_dispatch(w, abideDispatchNavigate, strdup(url));
}

// Maps a JSON `role` string to its enum; unknown/absent → none.
static AbideRole roleFromString(NSString *role) {
    if ([role isEqualToString:@"start"]) {
        return AbideRoleStart;
    }
    if ([role isEqualToString:@"disconnect"]) {
        return AbideRoleDisconnect;
    }
    return AbideRoleNone;
}
...
Start/Disconnect, gated by `role`), a `emit` item dispatches a abide:menu
...
*/
@interface AbideMenuAction : NSObject {
@public
    void *webviewHandle;
    ...
    AbideRole role;         // gating role for a Server navigate item; none otherwise
}
...
@implementation AbideMenuAction
...
// Dispatches a abide:menu CustomEvent (detail `{ name }`) into the page, so the
// app's own code handles it — including computing args for a parameterised call.
- (void)emit:(id)sender {
    NSString *js = [NSString
        stringWithFormat:
            @"window.dispatchEvent(new CustomEvent('abide:menu',{detail:{name:%@}}))",
            jsString(emitName)];
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
    if (navigateUrl != nil) {
        switch (role) {
            case AbideRoleStart:
                return g_abide_connected == 0;
            case AbideRoleDisconnect:
                return g_abide_connected != 0;
            default:
                return YES;
        }
...
static NSMenu *buildBundleMenu(NSDictionary *menuDef, void *webview_handle) {
    ...
    for (NSDictionary *item in menuDef[@"items"]) {
        ...
        AbideMenuAction *target = [[AbideMenuAction alloc] init];
...
*/
extern "C" ABIDE_EXPORT void abide_install_app_menu(void *webview_handle,
...
static const int kAbideBrowserController = 2;
...
static const char kAbideDownloadDestKey = 0;
...
blob:/data: downloads, and attachment responses — leaving every abide bundle app
...
@interface AbideDownloadDelegate : NSObject <WKNavigationDelegate, WKDownloadDelegate>
@end

@implementation AbideDownloadDelegate
...
- (void)download:(WKDownload *)download
    ...
    objc_setAssociatedObject(download, &kAbideDownloadDestKey, dest,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    completionHandler(dest);
}

- (void)downloadDidFinish:(WKDownload *)download {
    NSURL *dest = objc_getAssociatedObject(download, &kAbideDownloadDestKey);
    if (dest != nil) {
...
*/
extern "C" ABIDE_EXPORT void abide_install_downloads(void *webview_handle) {
    if (@available(macOS 11.3, *)) {
        WKWebView *webView =
            (WKWebView *)webview_get_native_handle(webview_handle, kAbideBrowserController);
        if (webView == nil) {
            return;
        }
        static AbideDownloadDelegate *delegate = nil;
        if (delegate == nil) {
            delegate = [[AbideDownloadDelegate alloc] init];
        }
