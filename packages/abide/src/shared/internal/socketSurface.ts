...
// The two reads (`live`/`peek`) and the six probes are INHERITED from `ReactiveProbeSurface` rather
// than re-listed here. They were
...
// each assign exactly ten members (the two reads, the six probes, `chunks` and `publish`). Folding the
// verbs in would widen the public surface with three
...
export interface SocketSurface<T, Args = void>
    ...
    // Narrows the shared `chunks(): unknown[] | undefined` to the message type. An ACTIVE READ
    // (client-sockets.md CS4.1) — reading it drives a subscription on the client, as `live` does and as
    // `peek`, which acquires nothing, deliberately does not.
