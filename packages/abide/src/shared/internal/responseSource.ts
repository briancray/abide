...
// The runtime payload a handler return resolves to after the memo sees through a transport wrapper: a
// stream helper → an AsyncIterable of its chunk; a json helper → its value; anything else unchanged.
//
// There is no outcome case, because there is no outcome VALUE: `error()`/`redirect()` throw, and a
// `throw` is `never`, which a union absorbs on its own — so `GET(({ fail }) => fail ? error(503) : { greeting })`
// infers `{ greeting }` with no brand to carry and no conditional to strip. The `OutcomeResponse` brand
// this once needed existed only to keep a RETURNED `Response` out of the handler's success type; making
// the helpers throw removed the thing it was hiding rather than hiding it better.
