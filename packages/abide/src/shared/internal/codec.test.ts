...
describe('canonicalKey', () => {
    ...
    test('is independent of object key order', () => {
        ...
        expect(canonicalKey({ x: { m: 1, n: 2 }, y: 3 })).toBe(
            canonicalKey({ y: 3, x: { n: 2, m: 1 } }),
        )
    })
    ...
    test('gives stable keys for Date, RegExp, URL', () => {
        ...
        expect(canonicalKey(new URL('https://x.test/p'))).toBe(
            canonicalKey(new URL('https://x.test/p')),
        )
    })
...
describe('encode / decode round-trip', () => {
    ...
    test('every TypedArray flavor', () => {
        ...
        for (const array of arrays) {
            ...
            expect(Array.from(decoded as Iterable<unknown>)).toEqual(
                Array.from(array as Iterable<unknown>),
            )
        }
    })
    ...
    test('nested Map with mixed key/value types', () => {
        ...
        const objKey = [...decoded.keys()].find((k) => typeof k === 'object') as Record<
            string,
            unknown
        >
...
describe('encode / decode references', () => {
    ...
    test('preserves a shared reference across Map and object', () => {
        ...
        const decoded = roundtrip({ obj: shared, map }) as {
            obj: object
