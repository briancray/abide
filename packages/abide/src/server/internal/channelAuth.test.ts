...
function publicProfile(): Rpc<{ id: string }, { id: string; open: boolean }> {
    return makeRead(
        'GET',
        async ({ id }: { id: string }): Promise<{ id: string; open: boolean }> => ({
            id,
            open: true,
        }),
        { cache: { shared: true } },
...
describe('channelAuth — @rpc: cache-channel join authorization', () => {
    ...
    test(
        ...
        async () => {
            ...
            if (!profile1) throw new Error('profile1 route was not registered')
            const allowed = (await profile1({ id: 'A' })) as { id: string; secret: string }
            ...
            const denied = (await profile1({ id: 'B' })) as { status: number }
    ...
    test(
        'anonymous WS — forbidden channels denied, public channel joins',
        async () => {
            const profile = guardedProfile()
