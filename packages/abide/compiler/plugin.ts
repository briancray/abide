...
// preload` for `bun test`, an `import '$compiler/preload.ts'` in `cli/internal/layers.ts` (and a
// `--preload` in `cli/internal/run.ts`) for the server lane, and `cli/internal/lane.ts`'s plugin
// list for the browser bundle. So the SAME compiler output runs in every lane — there is no build
