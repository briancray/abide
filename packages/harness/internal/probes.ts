// The two "what kind of thing is this?" probes the measurement half asks, and the harness's own copies of
// them on purpose.
//
// `$shared/internal/probes.ts` has both, and until the harness was a package of its own it imported them
// from there. It may not now, and not out of tidiness: `harness/measure` carries the invariant that
