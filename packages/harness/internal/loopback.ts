...
// handler's body from a real bundle. Those are asserted in `packages/dogfood/test/transport.test.ts`,
// which spawns a lane with no DOM emulator in it and serves for real.

// Both public: `dispatch` is what an app mounting abide in front of its own routes calls, and the
// harness is on the far side of a package boundary now — it reaches the framework by the same
// specifiers an app does.
import type { Wire } from 'abide/runtime/transport'
import { dispatch, websocket } from 'abide/server/internal'
