...
<p class="mt-2 text-slate-600">
    ...
    server binary, a launcher, and the native webview library travel together, so it runs on another
    machine of the same OS with nothing installed. It drives the OS webview over FFI (WebKit /
...
<section class="mt-6">
    ...
    <p class="mt-1 text-xs text-slate-500">
        ...
        live server, no connect-screen flash. The connect screen shows only when there's a real
        choice to make: first run, missing required config, or a dead/forgotten server.
    </p>
...
<section class="mt-6 rounded-lg border border-slate-200 bg-white p-5 text-sm text-slate-600">
    ...
    <p class="mt-1">
        ...
        slots map to the form: <code class="font-mono">title</code>
        → label,
        <code class="font-mono">description</code>
        → hint,
        <code class="font-mono">format: 'password'</code>
        → masked input,
        <code class="font-mono">default</code>
        → prefill. Answers persist to a per-user data-dir
...
<section class="mt-6 rounded-lg border border-slate-200 bg-white p-5 text-sm text-slate-600">
    ...
    <p class="mt-1">
        ...
        <code class="font-mono">emit</code>
        name. Subscribe with
...
<section class="mt-6 rounded-lg border border-slate-200 bg-white p-5 text-sm text-slate-600">
    ...
    <p class="mt-1">
        ...
        in a plain browser tab or a standalone server binary. Same name and meaning on both sides
        (the client reads the webview's init flag; the server reads the launcher's
        <code class="font-mono">BELTE_PARENT_PID</code>), so a page can branch desktop-only UI
...
<section class="mt-6 rounded-lg border border-slate-200 bg-white p-5 text-sm text-slate-600">
    ...
    <p class="mt-1">
        <code class="font-mono">belte bundle</code>
        wraps the server binary in a desktop launcher + webview for this platform. For just the
        embedded
        <em>server</em>
        executable (no window), use
        <code class="font-mono">belte compile</code>
        — the same binary the bundle spawns. On macOS, drop
        <code class="font-mono">src/bundle/icon.png</code>
        and the build converts it to
        <code class="font-mono">icon.icns</code>
        and wires the Info.plist. The finished
