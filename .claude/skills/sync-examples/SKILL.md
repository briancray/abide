---
name: sync-examples
description: After a belte API or README change, update every example (and the bundled scaffold template) so they compile, demonstrate the current API, and mirror the README's organisation. Use when the public surface changes (renamed export, moved directory, new helper, new section), when the README's structure shifts, or when example drift is suspected.
---
...
- `packages/belte/src` (with `packages/belte/package.json` `exports`) governs
  **behaviour and public surface**.
- `packages/belte/README.md` (the root `README.md` is a symlink to it) governs
...
If the README contradicts `packages/belte/src`, the README is stale — run
...
folder. (`cache` moved to `belte/shared`, but `pages/browser/cache/` survived
...
- **A component, never a nested `layout.svelte`.** belte resolves the deepest
...
1. `packages/belte/template/` — what `bunx @belte/belte scaffold` ships (its
...
2. `examples/scaffold/` — runnable workspace copy of the template (`src/`
   byte-identical; only `package.json`'s belte dep differs)
...
1. **Read `packages/belte/README.md` fresh** and outline it: sections, every
...
4. **Rebuild each demo page from the live code, not from its own old text.**
   ...
   implementation in `packages/belte/src` and write snippets and prose to
...
7. **Template ↔ scaffold parity** — `diff -ruN packages/belte/template/src
   examples/scaffold/src` must be empty (the generated `.belte/routes.d.ts`
   excepted; it's gitignored).
8. **Verify.**
   - `bun ../../packages/belte/bin/belte.ts build` exits 0 in each example;
     `bun --bun tsc --noEmit` clean in scaffold + kitchen-sink (barebones has
     no `.ts` files).
   - Import audit: every `@belte/belte/...` (or aliased belte import) across
     the four trees resolves to a current `exports` key:

     ```sh
     grep -rhoE "@belte/belte/[a-zA-Z/-]+" examples packages/belte/template --include="*.ts" --include="*.svelte" | sort -u
     ```
