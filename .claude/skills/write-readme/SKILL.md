...
- **`packages/abide/AGENTS.md`** — the exhaustive surface map. _Every_ `exports`
...
- ≤ 2 sentences on what abide is: typed RPCs fan out to HTTP, a CLI, an MCP,
  ...
  side. Built for humans _and_ machines.
...
- The gating note: a **typed input** unlocks the CLI and (for read-only methods,
  GET/HEAD) MCP — the handler's input type is projected to JSON Schema at build
  ...
  is not what flips the surfaces on). Do not say "a schema unlocks…". A mutating
  method (POST/PUT/PATCH/DELETE) never auto-exposes to MCP — it needs explicit
  `clients: { mcp: true }`. Note the consume forms: the **bare call
...
of which `isControlFlow.ts` lists the five _rendered_ ones and `snippet` is the
...
- `<script>` — imports (the rpc + socket via `$server/…`, a child component via
  ...
  `props` from `abide/ui/props`), `props()` destructure reads (a REQUIRED import
  like the others — a missing one is `Cannot find name 'props'`), and **every** reactive
...
- **async data reads — lead with the bare call, NOT `{#await}`.** The primary,
  documented way to read async data is the bare call in an interpolation
  (`{getMessages({ limit })}`): a _peek_ that reads `undefined` while pending (and
  auto-streams on SSR), so it composes with `?? fallback` / `?.` / `{#if}` /
  attributes with no block, paired with the `.pending()` / `.error()` probes for
  affordances (ADR-0032/0019; `{#await}` is "the opt-in for branch structure and
  `{:then}` narrowing" in the authors' words). Show, in this order: (1) a bare
  `{fn()?.field ?? fallback}` peek with a `fn.pending(...)` / `fn.error(...)` probe,
  (2) a `{await fn()}` inline read (blocks SSR until the value is in the initial
  HTML), then (3) the `{#await}…{/await}` block above framed as the EXPLICIT opt-in
  (distinct pending branch / local `{:catch}` / `{:then}` narrowing) — never as the
  default. The prose must say async reads have no ceremony and the bare call is the
  way; do not lead the async story with `{#await}`.
...
clearer. Its skeleton (re-derive the _content_ of each from the code; the order
...
- **Reactive state is reached through imported primitives**, resolved by import
  ...
  writes. It is read and reassigned as a **plain variable** (`count`, `count += 1`)
  in `.abide` source — the compiler desugars those to the cell's `.value`; do not
  present `.value` as the authoring surface in docs/snippets. The retired
  `scope().state(...)` /
  ...
  imported way. `props()` is the prop reader, a required import from
  `abide/ui/props` (`import { props } from '@abide/abide/ui/props'`; then
  `const { name = fallback, ...rest } = props()`) — not ambient, not `scope()`.
...
4. **Template coverage (README §4)** — the §4 component must demonstrate every
   ...
   parser is the single source for both):
    ...
    for t in '{#if' '{:else if' '{:else}' '{/if}' '{#for ' '{#for await' '{/for}' \
      '{await ' '.pending(' '.error(' '{#await' '{:then' '{:catch' '{:finally' \
      '{#switch' '{:case' '{:default' \
...
5. **Compile gate (authoritative — run this, do not skip)** — extract every
   ...
   is _present_, so a fence using REMOVED syntax (`<slot>`, `<template name>`,
